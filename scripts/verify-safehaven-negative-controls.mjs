...
const positiveReceiptPath = process.argv[3] ?? process.env.GOLDEN_POSITIVE_RECEIPT;
const oraclePath = process.argv[4] ?? "tests/golden/safehaven/oracle.json";
const verifierPath = process.argv[5] ?? "scripts/verify-safehaven-golden.mjs";
...
function runVerifier(receiptPath) {
  ...
  return {
    exit_code: execution.status === null ? 255 : execution.status,
    signal: execution.signal ?? null,
    stdout: execution.stdout ?? "",
    stderr: execution.stderr ?? "",
    result: parseVerifierOutput(execution.stdout),
  };
}
...
function orientationOf(trial, label) {
  ...
  if (!valid) {
    throw new Error(`${label} requires a valid A/B orientation to mutate coherently`);
  }
  return orientation;
}
function rawJudgeOf(trial, label) {
  ...
  if (!rawJudge || typeof rawJudge !== "object") {
    throw new Error(`${label} requires raw judge evidence to mutate coherently`);
  }
  return rawJudge;
}
...
function setTrialPreference(trial, normalizedPreference, label) {
  ...
  rawJudge.preference = rawPreferenceFor(normalizedPreference, orientation);
  trial.normalized_preference = normalizedPreference;
}
function setTrialDimension(trial, dimension, normalizedDelta, label) {
  const orientation = orientationOf(trial, label);
  const rawJudge = rawJudgeOf(trial, label);
  if (!rawJudge.dimensions || typeof rawJudge.dimensions !== "object") {
    throw new Error(`${label} requires raw judge dimensions to mutate coherently`);
  }
  rawJudge.dimensions[dimension] = rawDeltaFor(normalizedDelta, orientation);
...
function setPairVotes(pair, candidateVotes) {
  const trials = ensureArray(pair.trials, "visual pair trials");
  if (trials.length < 3) {
    throw new Error("negative-control setup requires 3 trials/pair");
  }
...
const CONTROLS = [
  {
    ...
    mutate(r) {
      ensureArray(r.competitive_landscape?.selected_donors, "selected donors").pop();
    },
  },
  {
    id: "NC-02",
    ...
    mutate(r) {
      const donors = ensureArray(r.competitive_landscape?.selected_donors, "selected donors");
      if (donors.length < 2) {
        throw new Error("NC-02 requires >=2 donors");
      }
      donors[1].normalized_domain = donors[0].normalized_domain;
    },
  },
  {
    id: "NC-03",
    ...
    mutate(r) {
      first(r.competitive_landscape?.selected_donors, "selected donors").class = "directory";
    },
  },
  {
    id: "NC-04",
    ...
    mutate(r) {
      first(r.donor_evidence, "donor evidence").screenshots = 0;
    },
  },
  {
    id: "NC-05",
    ...
    mutate(r) {
      const old = ensureArray(r.events, "events");
      const retained = old.filter((e) => {
  ...
  },
  {
    ...
    mutate(r) {
      ensureArray(r.seo_content_blueprint?.routes, "SEO routes").pop();
    },
  },
  {
    id: "NC-07",
    ...
    mutate(r) {
      const routes = ensureArray(r.seo_content_blueprint?.routes, "SEO routes");
      routes[routes.length - 1] = "/__oracle_unknown_route__/";
    },
  },
  {
    id: "NC-08",
    ...
    mutate(r) {
      r.seo_content_blueprint.competitive_landscape_ref += "-nc08-mismatch";
    },
  },
  {
    id: "NC-09",
  ...
  },
  {
    ...
    mutate(r) {
      r.structured_content.page_content_contract_ref += "-nc10";
    },
  },
  {
    id: "NC-11",
    ...
    mutate(r) {
      const route = first(r.structured_content?.route_results, "StructuredContent route results");
  ...
  },
  {
    ...
    mutate(r) {
      r.identity.seo_bot.llm_router_version = `${r.identity.seo_bot.llm_router_version}-drift`;
    },
  },
  {
    id: "NC-18",
    ...
    mutate(r) {
      first(
        r.llm_audit?.operations?.CONTENT_VALIDATION,
  ...
  },
  {
    ...
    mutate(r) {
      const pairs = ensureArray(r.visual?.pairs, "visual pairs");
      if (pairs.length !== 10) {
        throw new Error("NC-20 requires exactly 10 visual pairs");
      }
      pairs.forEach((pair, index) => {
        setPairVotes(pair, index < 6 ? 2 : 1);
      });
    },
  },
  {
    ...
    mutate(r) {
      const pair = ensureArray(r.visual?.pairs, "visual pairs").find(
        (p) => normalizeRoute(p.route) === "/" && p.viewport === "mobile",
      );
      if (!pair) {
        throw new Error("NC-21 requires /::mobile pair");
      }
      setPairVotes(pair, 1);
    },
  },
  {
    ...
    mutate(r) {
      ...
      for (const pair of pairs) {
        ensureArray(pair.trials, "visual trials").forEach((trial, index) => {
          setTrialDimension(
            trial,
            "visual_hierarchy",
            -1,
...
if (CONTROLS.length !== 25) {
  throw new Error(`negative-control inventory corrupt: ${CONTROLS.length}`);
}
const oracle = readJson(oraclePath);
const positive = readJson(positiveReceiptPath);
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "safehaven-golden-negative-"));
try {
  const base1 = runVerifier(path.resolve(ROOT, positiveReceiptPath));
  if (base1.exit_code !== 0 || base1.result?.verdict !== oracle.final_verdict.pass_name) {
    console.error(
      JSON.stringify(
        {
          schema: "l9.golden-negative-control-report/v1",
          verdict: "NEGATIVE_CONTROL_CALIBRATION_BLOCKED",
          reason: "BASE_RECEIPT_NOT_GOLDEN_PASS",
  ...
  const base2 = runVerifier(path.resolve(ROOT, positiveReceiptPath));
  const deterministicReplayPass =
    JSON.stringify(removeVolatile(base1.result)) === JSON.stringify(removeVolatile(base2.result));
  const controls = [];
  for (const control of CONTROLS) {
    ...
    const receiptPath = path.join(tempDir, `${control.id}.json`);
    fs.writeFileSync(receiptPath, `${JSON.stringify(mutated, null, 2)}\n`);
    const execution = runVerifier(receiptPath);
    const semanticReject =
      execution.exit_code === 1 &&
      execution.result &&
      execution.result.verdict !== oracle.final_verdict.pass_name;
    controls.push({
      id: control.id,
      expected: "FAIL",
      rejected: semanticReject,
      reason: control.reason,
      verifier_exit_code: execution.exit_code,
      verifier_verdict: execution.result?.verdict ?? null,
      failure_codes: [
        ...(execution.result?.hard_gate_failures ?? []),
        ...(execution.result?.blocking_inconclusive_states ?? []),
      ].map((f) => f.code),
      harness_error: execution.result
        ? null
        : execution.stderr.trim() || "verifier emitted no parseable result",
    });
  }
  const rejected = controls.filter((c) => c.rejected).length;
  const falseAcceptanceCount = controls.filter(
    (c) => c.verifier_exit_code === 0 || c.verifier_verdict === oracle.final_verdict.pass_name,
  ).length;
  const harnessErrors = controls.filter((c) => c.harness_error).length;
  const pass =
    rejected === 25 && falseAcceptanceCount === 0 && harnessErrors === 0 && deterministicReplayPass;
  const report = {
    schema: "l9.golden-negative-control-report/v1",
    oracle_id: oracle.oracle_id,
    positive_control: {
      expected: oracle.final_verdict.pass_name,
      actual: base1.result.verdict,
      pass: true,
    },
    negative_controls_total: 25,
    negative_controls_rejected: rejected,
    false_acceptance_count: falseAcceptanceCount,
    harness_error_count: harnessErrors,
    deterministic_replay_pass: deterministicReplayPass,
    controls,
    verdict: pass ? "GOLDEN_ORACLE_CALIBRATION_PASS" : "GOLDEN_ORACLE_CALIBRATION_FAIL",
  };
