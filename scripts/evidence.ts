...
const mode = (argValue("mode") ?? "end-to-end") as ExecutionMode;
...
if (command === "show") {
  ...
  print({
    rootDir,
    index,
    checkpoints: existsSync(resolve(rootDir, "checkpoints"))
      ? readdirSync(resolve(rootDir, "checkpoints")).sort()
      : [],
  });
  ...
  const result = {
    ...
    publication: publication
      ? {
          ...
          repository: publication.value.repository,
          branch: publication.value.branch,
          referenceValid: await store.verifyReference({
            kind: publication.record.kind,
            schema: publication.record.schema,
            logical_id: publication.record.logicalId,
            relative_path: publication.record.relativePath,
            sha256: publication.record.sha256,
          }),
        }
      : null,
    deployment: deployment
      ? {
          ...
          observedCommitSha: deployment.value.observedCommitSha,
          state: deployment.value.state,
          referenceValid: await store.verifyReference({
            kind: deployment.record.kind,
            schema: deployment.record.schema,
            logical_id: deployment.record.logicalId,
            relative_path: deployment.record.relativePath,
            sha256: deployment.record.sha256,
          }),
        }
      : null,
  ...
  const spec =
    argValue("spec") ?? "examples/supplemental-insurance-pros/domain_spec.normalized.yaml";
  ...
  const result = spawnSync(process.execPath, args, {
    stdio: json ? ["ignore", "inherit", "inherit"] : "inherit",
    env: process.env,
  });
