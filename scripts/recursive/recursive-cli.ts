...
function statePath(runId: string, relative: string): string {
  ...
  if (!absolute.startsWith(resolve(STATE_ROOT)))
    throw new Error("path escapes recursive state root");
  return absolute;
}
...
async function runRealE2E(specPath: string, runId: string): Promise<void> {
  ...
  const report = {
    command: "npm run pipeline:end-to-end",
  ...
  if ((result.status ?? 1) !== 0) {
    console.error(
      "[recursive] E2E failed; failure evidence is preserved for the engineering harvest",
    );
  }
}
...
async function commandImprove(args: string[]): Promise<void> {
  ...
  console.error(
    `[recursive] run ${runId} started; use 'npm run recursive:status -- --run ${runId}'`,
  );
}
...
async function commandStatus(args: string[]): Promise<void> {
  ...
  const entries = runId
    ? [runId]
    : [...readdirSync(root)].sort((left, right) => left.localeCompare(right));
