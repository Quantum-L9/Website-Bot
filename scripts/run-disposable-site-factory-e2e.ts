...
import { type BuildContext, makeBuildId } from "../src/pipeline/BuildContext.js";
...
const disposable = (label: string, value: string): string => {
  if (!/(?:disposable|throwaway|test)/i.test(value))
    throw new Error(`${label} must visibly identify a disposable/test target: ${value}`);
  return value;
};

const specPath = argument("spec") ?? "fixtures/ci-test-spec.yaml";
const repository = disposable(
  "WEBSITE_BOT_TEST_GITHUB_REPO",
  required("WEBSITE_BOT_TEST_GITHUB_REPO", process.env.WEBSITE_BOT_TEST_GITHUB_REPO),
);
const repositoryId = required(
  "WEBSITE_BOT_TEST_GITHUB_REPO_ID",
  process.env.WEBSITE_BOT_TEST_GITHUB_REPO_ID,
);
const projectId = disposable(
  "WEBSITE_BOT_TEST_VERCEL_PROJECT_ID",
  required("WEBSITE_BOT_TEST_VERCEL_PROJECT_ID", process.env.WEBSITE_BOT_TEST_VERCEL_PROJECT_ID),
);
...
const evidenceStore = new FileEvidenceStore({
  rootDir: resolve("build", "disposable-e2e-evidence", spec.client_id, buildId),
