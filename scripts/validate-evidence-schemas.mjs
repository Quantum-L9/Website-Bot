...
function compile(root) {
  ...
  return (value) => {
    const error = validate(root, value);
...
function sample(root, schema = root) {
  ...
  if (schema.type === "object") {
    const out = {};
    for (const key of schema.required ?? []) {
      out[key] = sample(root, schema.properties?.[key] ?? {});
    }
    return out;
  }
...
for (const [file, schemaConst] of Object.entries(required)) {
  ...
  if (doc.$schema !== "https://json-schema.org/draft/2020-12/schema")
    throw new Error(`${file}: wrong meta-schema`);
  if (doc.type !== "object" || doc.additionalProperties !== false)
    throw new Error(`${file}: object must fail closed`);
  ...
  scan(doc);
  if (camel.length)
    throw new Error(`${file}: non-snake_case persisted fields: ${[...new Set(camel)].join(", ")}`);
...
const extras = readdirSync(directory).filter(
  (name) => name.endsWith(".schema.json") && !(name in required),
);
console.log(
  JSON.stringify(
    { ok: true, draft: "2020-12", validated: results, additionalSchemas: extras.sort() },
    null,
    2,
  ),
);
