...
const clientLaunch =
  process.argv.includes("--client-launch") || process.env.LAUNCH_PHASE === "client_production";
...
const formProvider = String(process.env.FORM_PROVIDER ?? "")
  .trim()
  .toLowerCase();
if (clientLaunch && formProvider && formProvider !== "none") {
  secretsForLaunch.push("FORM_WEBHOOK_SECRET");
}
const crmProvider = String(process.env.CRM_PROVIDER ?? "")
  .trim()
  .toLowerCase();
...
if (clientLaunch) {
  if (!formProvider || formProvider === "none") {
    gateFailures.push(
      "FORM_PROVIDER must be a real provider (not none) for client production launch.",
    );
  }
...
if (!clientLaunch) {
  warnings.push(
    "Factory MVP mode: form/CRM/analytics/domain/legal client-launch gates are deferred (use --client-launch to enforce).",
  );
}
