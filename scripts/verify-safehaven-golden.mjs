 ...
 */
const IDENTIFIER_ALPHABET = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789._-";
...
export const STAGE_ALIASES = {
  "seo-build-intelligence-preflight": "seo-build-intelligence-preflight:PASS",
};
...
export function orderedSubsequenceViolations(requiredStages, events) {
  ...
  for (const stage of requiredStages) {
    ...
    if (index < 0)
      violations.push({ code: "REQUIRED_STAGE_MISSING", stage, expectedEventName: nameOf(stage) });
    else if (index <= lastIndex)
      violations.push({ code: "REQUIRED_STAGE_ORDER_VIOLATION", stage, index, lastIndex });
...
export function classifyVerdict(hardFailures, blockingInconclusiveStates) {
  if (hardFailures.length === 0 && blockingInconclusiveStates.length === 0)
    return "GOLDEN_E2E_PASS_IMPROVED";
...
function createVerifier(oracle) {
  ...
  function inconclusive(code, message, evidence = undefined, oracleIds = []) {
    blockingInconclusive.push({
      code,
      message,
      evidence,
      ...(oracleIds.length ? { oracle_ids: oracleIds } : {}),
    });
  }
  ...
  function normalizedSet(values) {
    return new Set(
      (values ?? []).map((v) => (typeof v === "string" ? normalizedRoute(v) : String(v))),
    );
  }
  function requireExactSet(actual, expected, code, message) {
    ...
    if (missing.length || extra.length)
      fail(code, message, { missing, extra, actualCount: a.size, expectedCount: e.size });
  }
  ...
  function requireExactRouteMultiset(actual, expected, code, message, duplicateCode) {
    ...
    for (const [route, copies] of duplicates) {
      fail(duplicateCode, `${route} appears ${copies + 1} times in the route set`, {
        route,
        extraCopies: copies,
      });
    }
  ...
  function requireBefore(events, firstName, secondName, code) {
    ...
    if (first < 0 || second < 0 || first >= second)
      fail(code, `${firstName} must occur before ${secondName}`, {
        firstIndex: first,
        secondIndex: second,
      });
  }
  ...
  function verify(receipt, testCase) {
    ...
    for (const repo of ["website_bot", "seo_bot", "llm_router"])
      requireNonEmpty(
        receipt.identity?.[repo]?.sha,
        "IDENTITY_SHA_MISSING",
        `${repo} full SHA missing`,
      );
    ...
    requireEq(
      ...
      "Consumer Router version must equal tested Router package identity",
    );
    ...
    for (const repo of ["website_bot", "seo_bot", "llm_router"]) {
      ...
      if (state === undefined || state === null)
        fail("WORKTREE_STATE_MISSING", `${repo} worktree_state evidence missing`);
      else if (state === "CLEAN") {
        /* accepted */
      } else if (
        typeof state === "object" &&
        ...
        fail(
          ...
          `${repo} worktree is DIRTY without a recorded deterministic dirty-state identity`,
          state,
        );
      }
    }
    ...
    if (botInterop === undefined || botInterop === null) {
      ...
      requireNonEmpty(
        botInterop.website_bot_version,
        "BOT_INTEROP_EVIDENCE_MISSING",
        "bot interop website_bot_version missing",
      );
      requireNonEmpty(
        botInterop.seo_bot_version,
        "BOT_INTEROP_EVIDENCE_MISSING",
        "bot interop seo_bot_version missing",
      );
      if (botInterop.compatible !== true)
        fail("BOT_INTEROP_MISMATCH", "bot interop compatibility not proven", botInterop.compatible);
    }

    /* ---------------- EXECUTION GRAPH (ORACLE-008..012) ---------------- */
    const execGraph = oracle.execution_graph ?? {};
    const buildIntent = oracleString(
      execGraph,
      "build_intent",
      "WRONG_BUILD_INTENT",
      "execution_graph.build_intent",
    );
    if (buildIntent !== undefined)
      requireEq(
        receipt.run?.build_intent,
        buildIntent,
        "WRONG_BUILD_INTENT",
        "Golden run must execute REDESIGN_IMPROVE",
      );
    const copyFallbackAllowed = oracleBool(
      execGraph,
      "copy_fallback_used",
      "COPY_FALLBACK_USED",
      "execution_graph.copy_fallback_used",
    );
    if (copyFallbackAllowed !== undefined)
      requireFalse(
        receipt.run?.copy_fallback_used,
        "COPY_FALLBACK_USED",
        "COPY fallback is forbidden",
      );
    const genericFallbackAllowed = oracleBool(
      execGraph,
      "generic_fallback_used",
      "GENERIC_FALLBACK_USED",
      "execution_graph.generic_fallback_used",
    );
    if (genericFallbackAllowed !== undefined)
      requireFalse(
    ...
    const requiredStages = oracleNonEmptyArray(
      execGraph,
    ...
    if (requiredStages !== undefined) {
      for (const violation of orderedSubsequenceViolations(requiredStages, events)) {
        fail(
          violation.code,
          violation.code === "REQUIRED_STAGE_MISSING"
            ? `required stage ${violation.stage} missing`
            : `stage ${violation.stage} executed out of order`,
          {
            stage: violation.stage,
            expectedEventName: violation.expectedEventName,
            index: violation.index,
            lastIndex: violation.lastIndex,
          },
        );
      }
    }
    ...
    const forbiddenStages = oracleNonEmptyArray(
      execGraph,
      "forbidden_stages_under_redesign",
      ...
      for (const event of events) {
        ...
        for (const forbidden of forbiddenStages) {
          if (name === forbidden || name.startsWith(`${forbidden}:`)) {
            fail(
              "FORBIDDEN_REDESIGN_STAGE_EXECUTED",
              `forbidden stage ${forbidden} executed under REDESIGN_IMPROVE`,
              event,
            );
          }
        }
    ...
    const requiredChecks = oracleNonEmptyArray(
      oracle.preflight ?? {},
      "required_checks",
        ...
        for (const check of checks) {
          ...
          if (seen.has(name))
            fail(
              ...
              `preflight check ${name} recorded more than once`,
              name,
            );
          seen.add(name);
          if (status !== "PASS")
            fail("PREFLIGHT_CHECK_FAILED", `preflight check ${name} did not PASS`, status);
        }
        for (const required of requiredChecks) {
          if (!seen.has(required))
            fail(
              "PREFLIGHT_CHECK_MISSING",
              `required preflight check ${required} missing`,
              required,
            );
        }
      }
    ...
    const selectedDonorCount = oracleNumber(
      oracleCompetitive,
      "selected_donor_count",
      "COMPETITIVE_EVIDENCE_INCOMPLETE",
      "competitive_landscape.selected_donor_count",
    );
    const forbiddenDonorClasses = oracleNonEmptyArray(
      oracleCompetitive,
      "forbidden_selected_classes",
      "FORBIDDEN_DONOR_CLASS",
      "competitive_landscape.forbidden_selected_classes",
    );
    if (selectedDonorCount !== undefined)
      requireEq(
        competitive.selected_donors?.length,
        selectedDonorCount,
        "COMPETITIVE_EVIDENCE_INCOMPLETE",
        `Exactly ${selectedDonorCount} selected donors required`,
      );
    ...
    requireEq(
      ...
      "Selected donors must be ten unique normalized companies",
    );
    requireTrue(
      competitive.evidence_complete,
      "COMPETITIVE_EVIDENCE_NOT_COMPLETE",
      "CompetitiveLandscape must be complete",
    );
    requireEq(
      competitive.ranking_llm_calls,
      0,
      "COMPETITIVE_RANKING_LLM_USED",
      "Competitive rank authority must be deterministic",
    );
    for (const donor of competitive.selected_donors ?? []) {
      ...
      if (!(Number(donor.rank) >= 1))
        fail("DONOR_RANK_INVALID", `Invalid donor rank: ${donor.normalized_domain}`, donor.rank);
      if (!(Number(donor.visibility_contribution) >= 0))
        fail("DONOR_VISIBILITY_INVALID", `Visibility evidence missing: ${donor.normalized_domain}`);
      // ORACLE-018 forbidden_selected_classes
      const donorClass = donor.class;
      if (
        donorClass === undefined ||
        donorClass === null ||
        (typeof donorClass === "string" && donorClass.trim() === "")
      ) {
        ...
        fail(
          "FORBIDDEN_DONOR_CLASS",
          `Forbidden donor class ${donorClass}: ${donor.normalized_domain}`,
          donorClass,
        );
      }
    }
    ...
    const acceptedDonors = oracleNumber(
      oracle.donor_evidence ?? {},
      "accepted_donors",
      "DONOR_EVIDENCE_INCOMPLETE",
      "donor_evidence.accepted_donors",
    );
    if (acceptedDonors !== undefined)
      requireEq(
        donorEvidence?.length,
        acceptedDonors,
        "DONOR_EVIDENCE_INCOMPLETE",
        `${acceptedDonors} donor evidence sets required`,
      );
    if (!Array.isArray(donorEvidence)) {
      ...
      for (const donor of donorEvidence) {
        if (typeof donor.successful_pages !== "number" || donor.successful_pages < 1)
          fail(
            "DONOR_CRAWL_INCOMPLETE",
            `No successful page for ${donor.domain}`,
            donor.successful_pages,
          );
        if (typeof donor.screenshots !== "number" || donor.screenshots < 1)
          fail(
            "DONOR_SCREENSHOT_INCOMPLETE",
            `No screenshot evidence for ${donor.domain}`,
            donor.screenshots,
          );
        requireNonEmpty(
        ...
        if (typeof timestamp !== "string" || timestamp.trim() === "")
          fail("DONOR_TIMESTAMP_MISSING", `Crawl timestamp missing for ${donor.domain}`, timestamp);
        else if (Number.isNaN(Date.parse(timestamp)))
          fail(
            "DONOR_TIMESTAMP_INVALID",
            `Crawl timestamp unparseable for ${donor.domain}`,
            timestamp,
          );
      }
    }
    ...
    if (websiteBlueprint === undefined || websiteBlueprint === null) {
      ...
      requireNonEmpty(
        websiteBlueprint.artifact_ref,
        "WEBSITE_BLUEPRINT_INVALID",
        "WebsiteBuildBlueprint artifact_ref missing",
      );
      const visualRequirements = websiteBlueprint.visual_requirements;
      const empty =
        visualRequirements === undefined ||
        visualRequirements === null ||
        (Array.isArray(visualRequirements) && visualRequirements.length === 0) ||
        (typeof visualRequirements === "object" &&
          !Array.isArray(visualRequirements) &&
          Object.keys(visualRequirements).length === 0);
      if (empty)
        fail(
          "BLUEPRINT_VISUAL_REQUIREMENTS_MISSING",
          "WebsiteBuildBlueprint visual_requirements missing or empty",
        );
    }
    ...
    requireExactRouteMultiset(
      seoBlueprint.routes,
      expectedRoutes,
      "ROUTE_SET_MISMATCH",
      "SEOContentBlueprint must contain the exact frozen Safe Haven route set",
      "SEO_BLUEPRINT_DUPLICATE_ROUTE",
    );
    const oracleSeo = oracle.seo_content_blueprint ?? {};
    const batchSize = oracleNumber(
      oracleSeo,
      "batch_size",
    ...
    if (batchSize !== undefined)
      requireEq(
        seoBlueprint.batch_size,
        batchSize,
        "SEO_BATCH_SIZE_DRIFT",
        "SEOContentBlueprint batch size must remain deterministic",
      );
    const batchCount = oracleNumber(
      oracleSeo,
    ...
    if (batchCount !== undefined)
      requireEq(
        seoBlueprint.batch_count,
        batchCount,
        "SEO_BATCH_COUNT_INVALID",
        "29 routes at batch size four must use eight batches",
      );

    const landscapeRef = competitive.artifact_ref;
    requireEq(
      receipt.website_build_blueprint?.competitive_landscape_ref,
      landscapeRef,
      "WEBSITE_BLUEPRINT_LANDSCAPE_MISMATCH",
      "WebsiteBuildBlueprint must reference exact landscape",
    );
    requireEq(
      seoBlueprint.competitive_landscape_ref,
      landscapeRef,
    ...
    requireEq(
      seoBlueprint.unknown_content_slots,
      0,
      "SEO_BLUEPRINT_UNKNOWN_CONTENT_SLOT",
      "SEOContentBlueprint unknown content slots must be zero",
    );
    // ORACLE-035 invalid_internal_link_targets (presence required)
    requireEq(
      seoBlueprint.invalid_internal_link_targets,
      0,
      "SEO_BLUEPRINT_INVALID_INTERNAL_LINK_TARGET",
      "SEOContentBlueprint internal link targets must all be authorized",
    );
    ...
    requireExactSet(
      ...
      expectedRoutes,
      "ROUTE_SET_MISMATCH",
      "PageContentContract must contain the exact frozen Safe Haven route set",
    );
    requireEq(pcc.llm_calls, 0, "PCC_LLM_USED", "PageContentContract must use zero LLM calls");
    requireEq(
      pcc.unplaced_requirements,
      0,
      "CONTENT_REQUIREMENT_UNPLACED",
      "All required SEO content requirements must be placed",
    );
    // ORACLE-039 invalid_business_facts (presence required)
    requireEq(
      pcc.invalid_business_facts,
      0,
      "PCC_INVALID_BUSINESS_FACT",
      "PageContentContract invalid business facts must be zero",
    );
    ...
    if (determinism === undefined || determinism === null) {
      ...
      if (
        typeof digestRun1 !== "string" ||
        digestRun1.trim() === "" ||
        typeof digestRun2 !== "string" ||
        digestRun2.trim() === ""
      ) {
        fail("PCC_NONDETERMINISTIC", "PCC determinism digest evidence missing", {
          digest_run_1: digestRun1,
          digest_run_2: digestRun2,
        });
      } else if (digestRun1 !== digestRun2) {
        fail("PCC_NONDETERMINISTIC", "PCC digest differs between identical semantic inputs", {
          digest_run_1: digestRun1,
          digest_run_2: digestRun2,
        });
      }
    ...
    requireExactSet(
      ...
      expectedRoutes,
      "ROUTE_SET_MISMATCH",
      "StructuredContentPackage must contain the exact frozen Safe Haven route set",
    );
    requireEq(
      structuredContent.page_content_contract_ref,
      pcc.artifact_ref,
    ...
    if (!Array.isArray(routeResults)) {
      ...
      fail(
        ...
        "structured_content.route_results evidence missing",
      );
      ...
      for (const route of routeResults) {
        if (typeof route.schema_errors !== "number" || route.schema_errors !== 0)
          fail(
            "STRUCTURED_CONTENT_SCHEMA_INVALID",
            `Schema-invalid structured content: ${route.route_id}`,
            route.schema_errors,
          );
        if (typeof route.unsupported_claims !== "number" || route.unsupported_claims !== 0)
          fail(
            "UNSUPPORTED_CONTENT_CLAIM",
            `Unsupported content claim: ${route.route_id}`,
            route.unsupported_claims,
          );
        if (typeof route.failed_requirements !== "number" || route.failed_requirements !== 0)
          fail(
            "CONTENT_REQUIREMENT_UNSATISFIED",
            `Unsatisfied content requirements: ${route.route_id}`,
            route.failed_requirements,
          );
        if (typeof route.repair_attempts !== "number" || route.repair_attempts > (maxRepairs ?? 1))
          fail(
            "CONTENT_REPAIR_BUDGET_EXCEEDED",
            `${route.route_id} used more than one repair`,
            route.repair_attempts,
          );
        if (
          typeof route.generation_calls !== "number" ||
          route.generation_calls > (maxGenerationCalls ?? 2)
        )
          fail(
            "CONTENT_GENERATION_BUDGET_EXCEEDED",
            `${route.route_id} used more than two generation calls`,
            route.generation_calls,
          );
        if (typeof route.prose_without_blocks !== "number" || route.prose_without_blocks !== 0)
          fail(
            "STRUCTURED_CONTENT_BLOCKS_REQUIRED",
            `Prose outside blocks: ${route.route_id}`,
        ...
        for (const section of route.sections ?? []) {
          if (section !== null && typeof section === "object") {
            for (const key of Object.keys(section))
              if (forbiddenSectionAliases?.includes(key)) aliasFields.push(key);
          }
        }
        for (const alias of Array.isArray(route.section_alias_fields)
          ? route.section_alias_fields
          : []) {
        ...
        if (aliasFields.length)
          fail(
            "STRUCTURED_CONTENT_FORBIDDEN_SECTION_ALIAS",
            `Forbidden section alias field(s): ${[...new Set(aliasFields)].join(", ")} on ${route.route_id}`,
            [...new Set(aliasFields)],
          );
      }
    }
    ...
    requireEq(
      legacy.content_generation_calls,
      0,
      "LEGACY_CONTENT_AUTHORITY_USED",
      "Legacy Website-Bot content generation is forbidden",
    );
    requireEq(
      legacy.schema_llm_calls,
      0,
      "LEGACY_SCHEMA_AUTHORITY_USED",
      "LLM schema generation is forbidden on redesign",
    );
    ...
    requireEq(
      businessTruth.prohibition_violations,
      0,
      "BUSINESS_PROHIBITION_VIOLATION",
      "Candidate violates a frozen business prohibition",
    );

    /* ---------------- SOURCE ASSETS (ORACLE-054..061) ---------------- */
    const assets = receipt.assets ?? {};
    if ((assets.raw_source_images ?? 0) < 1)
      fail("SOURCE_ASSET_CORPUS_EMPTY", "Source asset harvesting found no images");
    if ((assets.authorized_reusable_images ?? 0) < 1)
      fail(
        "AUTHORIZED_SOURCE_ASSETS_MISSING",
        "Safe Haven authorization produced no reusable images",
      );
    if ((assets.selected_source_images ?? 0) < 1)
      fail("SOURCE_IMAGE_REUSE_MISSING", "No authorized Safe Haven source image was selected");
    requireEq(
    ...
    if (!Array.isArray(candidateDispositions)) {
      fail(
        "FORBIDDEN_CANDIDATE_ASSET_DISPOSITION",
        "assets.candidate_dispositions evidence missing",
      );
    } else {
      for (const disposition of candidateDispositions) {
        if (forbiddenDispositions?.includes(disposition))
          fail(
            "FORBIDDEN_CANDIDATE_ASSET_DISPOSITION",
            `Forbidden candidate asset disposition: ${disposition}`,
            disposition,
          );
      }
    }
    ...
    if (
      typeof eligibleProof !== "number" ||
      typeof selectedProof !== "number" ||
      typeof projectProofRequired !== "boolean"
    ) {
      fail(
        "REQUIRED_SOURCE_PROJECT_PROOF_NOT_SELECTED",
        "project-proof source asset counts evidence missing",
        {
          eligible_source_project_proof_count: eligibleProof,
          selected_source_project_proof_count: selectedProof,
          project_proof_required: projectProofRequired,
        },
      );
    } else if (eligibleProof > 0 && projectProofRequired === true && !(selectedProof > 0)) {
      fail(
        "REQUIRED_SOURCE_PROJECT_PROOF_NOT_SELECTED",
        "eligible required source project-proof photography disappeared",
        {
          eligible_source_project_proof_count: eligibleProof,
          selected_source_project_proof_count: selectedProof,
        },
    ...
    if (
      typeof eligibleGallery !== "number" ||
      typeof selectedGallery !== "number" ||
      typeof galleryRequired !== "boolean"
    ) {
      fail("REQUIRED_SOURCE_GALLERY_NOT_SELECTED", "gallery source asset counts evidence missing", {
        eligible_source_gallery_count: eligibleGallery,
        selected_source_gallery_count: selectedGallery,
      ...
      fail(
        ...
        "eligible required source gallery photography disappeared",
        {
          eligible_source_gallery_count: eligibleGallery,
          selected_source_gallery_count: selectedGallery,
        },
    ...
    requireEq(
      receipt.site?.routes?.length,
      29,
      "SITE_ROUTE_COUNT_MISMATCH",
      "Candidate must build all 29 routes",
    );
    requireEq(
      receipt.site?.reachable_routes,
      29,
      "SITE_REACHABILITY_INCOMPLETE",
      "All 29 routes must be reachable",
    );
    requireEq(
    ...
    requireExactSet(
      ...
      expectedRoutes,
      "ROUTE_SET_MISMATCH",
      "BuiltSite must contain the exact frozen Safe Haven route set",
    );
    ...
    if (perRoute === undefined || perRoute === null || typeof perRoute !== "object") {
      ...
      const entryFor = (route) =>
        Array.isArray(perRoute)
          ? perRoute.find(
              (entry) => entry !== null && normalizedRoute(entry.route) === normalizedRoute(route),
            )
      ...
      for (const route of expectedRoutes) {
        ...
        if (reqHttp200 && entry.http_status !== 200)
          fail(
            "ROUTE_HTTP_STATUS_INVALID",
            `${route} returned ${entry.http_status}`,
            entry.http_status,
          );
        if (reqSingleH1 && entry.h1_count !== 1)
          fail(
            "ROUTE_H1_COUNT_INVALID",
            `${route} has ${entry.h1_count} h1 elements`,
            entry.h1_count,
          );
        if (reqTitle && entry.title_present !== true)
          fail("ROUTE_TITLE_MISSING", `${route} has no title`, entry.title_present);
        if (reqMeta && entry.meta_description_present !== true)
          fail(
            "ROUTE_META_DESCRIPTION_MISSING",
            `${route} has no meta description`,
            entry.meta_description_present,
          );
        if (reqCanonical && entry.canonical_present !== true)
          fail("ROUTE_CANONICAL_MISSING", `${route} has no canonical`, entry.canonical_present);
        if (reqLang && entry.lang_present !== true)
          fail("ROUTE_LANG_MISSING", `${route} has no lang attribute`, entry.lang_present);
        if (typeof entry.title === "string" && entry.title.trim() !== "") {
          titles.add(entry.title.trim());
          nonEmptyTitles++;
        }
        if (typeof entry.canonical === "string" && entry.canonical.trim() !== "") {
          canonicals.add(entry.canonical.trim());
          nonEmptyCanonicals++;
        }
      }
      if (nonEmptyTitles !== expectedRoutes.length || titles.size !== expectedRoutes.length) {
        fail(
          ...
          `expected ${expectedRoutes.length} unique non-empty page titles, got ${titles.size} unique / ${nonEmptyTitles} non-empty`,
          { unique: titles.size, nonEmpty: nonEmptyTitles },
        );
      }
      if (
        nonEmptyCanonicals !== expectedRoutes.length ||
        canonicals.size !== expectedRoutes.length
      ) {
        fail(
          "DUPLICATE_CANONICAL_URL",
          `expected ${expectedRoutes.length} unique non-empty canonical URLs, got ${canonicals.size} unique / ${nonEmptyCanonicals} non-empty`,
          { unique: canonicals.size, nonEmpty: nonEmptyCanonicals },
        );
      }
    }
    ...
    requireEq(
      llmAudit.direct_provider_bypass_count,
      0,
      "PROVIDER_BYPASS_DETECTED",
      "All governed LLM operations must use LLM-Router",
    );
    // ORACLE-078 unsupported_capability_combination_count (presence required)
    requireEq(
      llmAudit.unsupported_capability_combination_count,
      0,
      "UNSUPPORTED_LLM_CAPABILITY_COMBINATION",
      "Unsupported LLM capability combinations are forbidden",
    );
    ...
    if (operations === undefined || operations === null || typeof operations !== "object") {
      ...
      for (const operation of [
        ...
        if (!Array.isArray(calls)) {
          ...
          for (const call of calls) {
            requireFalse(
              call.searchRequired,
              "UNEXPECTED_SEARCH_ROUTING",
              `${operation} unexpectedly required search`,
            );
            requireEq(
              call.searchPolicySource,
              "EXPLICIT",
              "SEARCH_POLICY_NOT_EXPLICIT",
              `${operation} must explicitly suppress search`,
            );
          }
        }
      }
      ...
      if (!Array.isArray(visualQACalls)) {
        ...
        for (const call of visualQACalls)
          requireFalse(
            call.searchRequired,
            "VISUAL_QA_ROUTING_POLICY_VIOLATION",
            "VISUAL_QA must explicitly suppress search",
          );
      }
    }
    ...
    const requiredPairs = oracleNumber(
    ...
    if (pairs === null || (requiredPairs !== undefined && pairs.length !== requiredPairs)) {
      fail(
        "VISUAL_CAPTURE_INCOMPLETE",
        `Five routes × two viewports = ${requiredPairs} visual pairs`,
        { actual: pairs?.length, required: requiredPairs },
      );
      ...
      inconclusive(
        ...
        "trial evidence missing; visual verdict inconclusive",
        { pairs: pairs?.length ?? 0 },
        ["ORACLE-095"],
      );
      inconclusive(
        "VISUAL_PAIR_NO_MAJORITY",
        "pair evidence missing; visual verdict inconclusive",
        { pairs: pairs?.length ?? 0 },
        ["ORACLE-096"],
      );
      inconclusive(
        "VISUAL_WILSON_INTERVAL_INCONCLUSIVE",
        "vote evidence missing; visual verdict inconclusive",
        { pairs: pairs?.length ?? 0 },
        ["ORACLE-097"],
      );
      inconclusive(
        "VISUAL_WEIGHTED_MEAN_DELTA_INSUFFICIENT",
        "candidate delta evidence missing; visual verdict inconclusive",
        { pairs: pairs?.length ?? 0 },
        ["ORACLE-092"],
      );
      inconclusive(
        "VISUAL_DIMENSION_MISSING",
        "dimension evidence missing; visual verdict inconclusive",
        { pairs: pairs?.length ?? 0 },
        ["ORACLE-093"],
      );
    } else {
      const criticalPairs = oracle.visual_oracle?.pass?.critical_pairs_may_not_lose;
      if (!Array.isArray(criticalPairs) || criticalPairs.length === 0)
        fail(
          "VISUAL_ORACLE_CONFIG_INVALID",
          "oracle critical_pairs_may_not_lose configuration missing",
        );
      const criticalDimensions = oracle.visual_oracle?.pass?.critical_dimensions_may_not_regress;
      if (!Array.isArray(criticalDimensions) || criticalDimensions.length === 0)
        fail(
          "VISUAL_ORACLE_CONFIG_INVALID",
          "oracle critical_dimensions_may_not_regress configuration missing",
        );
      const weights = oracle.visual_oracle?.dimensions;
      const oracleVisualPass = oracle.visual_oracle?.pass ?? {};
      const minWins = oracleNumber(
        oracleVisualPass,
        "minimum_pair_majority_wins",
        "VISUAL_IMPROVEMENT_INSUFFICIENT",
        "visual_oracle.pass.minimum_pair_majority_wins",
      );
      const maxLosses = oracleNumber(
        oracleVisualPass,
        "maximum_pair_majority_losses",
        "VISUAL_REGRESSION_TOO_BROAD",
        "visual_oracle.pass.maximum_pair_majority_losses",
      );
      const minVotes = oracleNumber(
        oracleVisualPass,
        "minimum_candidate_votes",
        "VISUAL_VOTE_CONFIDENCE_INSUFFICIENT",
        "visual_oracle.pass.minimum_candidate_votes",
      );
      const wilsonMinimum = oracleNumber(
        oracleVisualPass,
        "wilson_lower_bound_must_exceed",
        "VISUAL_WILSON_INTERVAL_INCONCLUSIVE",
        "visual_oracle.pass.wilson_lower_bound_must_exceed",
      );
      const weightedMeanThreshold = oracleNumber(
        oracleVisualPass,
      ...
      for (const pair of pairs) {
        ...
        if (pair.candidate_blank !== false)
          fail("CANDIDATE_BLANK_CAPTURE", `Blank candidate capture: ${key}`, pair.candidate_blank);
        if (pair.baseline_blank !== false)
          fail("BASELINE_BLANK_CAPTURE", `Blank baseline capture: ${key}`, pair.baseline_blank);
        if (pair.route_match !== true)
          fail("VISUAL_ROUTE_MISMATCH", `Cross-route visual pair: ${key}`, pair.route_match);
        if (pair.viewport_match !== true)
          fail(
            "VISUAL_VIEWPORT_MISMATCH",
            `Mismatched viewport visual pair: ${key}`,
            pair.viewport_match,
          );
        const runId = receipt.run?.run_id;
        if (typeof runId !== "string" || runId.trim() === "")
          fail("STALE_VISUAL_CAPTURE", "run.run_id evidence missing", runId);
        else if (pair.captured_run_id !== runId)
          fail("STALE_VISUAL_CAPTURE", `Stale capture: ${key}`, {
            captured_run_id: pair.captured_run_id,
        ...
        if (!Array.isArray(trials) || trials.length !== 3) {
          ...
          inconclusive(
            "VISUAL_ORACLE_MISSING_TRIAL",
            `${key} missing a blind trial; visual verdict inconclusive`,
            { pair: key, trials: trials?.length },
            ["ORACLE-095"],
          );
          continue;
        }
        ...
        for (const trial of trials) {
          ...
          if (trial.blind !== true && trial.blind !== false)
            fail("VISUAL_BLINDING_EVIDENCE_MISSING", `${key} trial lacks blind flag`, trial.blind);
          else if (trial.blind !== true)
            fail("VISUAL_JUDGE_NOT_BLIND", `${key} trial judge was not blind`, trial.blind);
          const manifest = trial.judge_input_manifest;
          if (
            manifest === undefined ||
            manifest === null ||
            (typeof manifest === "string" && manifest.trim() === "") ||
            ...
            fail(
              ...
              `${key} trial lacks judge_input_manifest`,
              manifest,
            );
          } else {
            ...
            if (leaks.length > 0)
              fail(
                ...
                `${key} judge input leaks candidate/baseline identity`,
                leaks,
              );
          }
          ...
          if (
            ...
            for (const [dimension, value] of Object.entries(delta)) {
              ...
              if (!Number.isFinite(numeric))
                fail("VISUAL_DIMENSION_MISSING", `non-finite ${dimension} delta in ${key}`, value);
        ...
        // ORACLE-096 judge_disagreement_without_majority — blocking inconclusive
          ...
          inconclusive(
            "VISUAL_PAIR_NO_MAJORITY",
            `${key} blind trials did not resolve to a majority; visual verdict inconclusive`,
            { pair: key, candidate: pairCandidateVotes, baseline: pairBaselineVotes },
            ["ORACLE-096"],
          );

        // ORACLE-090 critical_pairs_may_not_lose (dynamic oracle list)
        if (criticalPairs?.includes(key) && pairBaselineVotes >= 2)
          fail("CRITICAL_VISUAL_PAIR_REGRESSED", `${key} lost the blind pairwise comparison`);
      }
      ...
      if (minWins !== undefined && majorityWins < minWins)
        fail(
          "VISUAL_IMPROVEMENT_INSUFFICIENT",
          `Candidate must win at least ${minWins}/10 pair majorities`,
          { majorityWins },
        );
      if (maxLosses !== undefined && majorityLosses > maxLosses)
        fail(
          "VISUAL_REGRESSION_TOO_BROAD",
          `Candidate may lose at most ${maxLosses}/10 visual pairs`,
          { majorityLosses },
        );
      if (minVotes !== undefined && candidateVotes < minVotes)
        fail(
          "VISUAL_VOTE_CONFIDENCE_INSUFFICIENT",
          `Candidate must receive at least ${minVotes}/30 blind votes`,
      ...
      if (wilsonMinimum !== undefined && !(lowerBound > wilsonMinimum)) {
        inconclusive(
          "VISUAL_WILSON_INTERVAL_INCONCLUSIVE",
          `95% Wilson lower bound ${lowerBound.toFixed(4)} does not strictly exceed ${wilsonMinimum}; visual improvement unproven`,
          {
            candidateVotes,
            totalVotes,
            lowerBound,
            required: wilsonMinimum,
            pass_gate: "VISUAL_CONFIDENCE_INTERVAL_INCONCLUSIVE",
          },
          ["ORACLE-089", "ORACLE-097"],
        );
      }
      ...
      if (
        ...
        inconclusive(
          "VISUAL_DIMENSION_MISSING",
          "oracle visual_oracle.dimensions configuration missing; visual verdict inconclusive",
          undefined,
          ["ORACLE-093"],
        );
        inconclusive(
          "VISUAL_WEIGHTED_MEAN_DELTA_INSUFFICIENT",
          "dimension weight evidence missing; weighted mean delta unproven",
          undefined,
          ["ORACLE-092"],
        );
      } else {
        ...
        if (Math.abs(weightSum - 1.0) > 1e-9)
          fail(
            ...
            `visual dimension weights must sum to 1.0, got ${weightSum}`,
        ...
        for (const pair of pairs) {
          for (const trial of Array.isArray(pair.trials) ? pair.trials : []) {
            ...
            if (
              ...
              fail(
                "VISUAL_DIMENSION_MISSING",
                `${criticalPairKey(pair.route, pair.viewport)} trial lacks normalized_candidate_delta`,
                delta,
              );
              continue;
            }
            for (const dimension of Object.keys(weights)) {
              if (!(dimension in delta))
                fail(
                  "VISUAL_DIMENSION_MISSING",
                  `trial delta missing configured dimension ${dimension}`,
                  { pair: criticalPairKey(pair.route, pair.viewport), dimension },
                );
            }
          }
        }
        ...
        if (weightedMean === null) {
          inconclusive(
            "VISUAL_WEIGHTED_MEAN_DELTA_INSUFFICIENT",
            "candidate delta evidence missing; weighted mean delta unproven",
            undefined,
            ["ORACLE-092"],
          );
          ...
          inconclusive(
            "VISUAL_WEIGHTED_MEAN_DELTA_INSUFFICIENT",
            "oracle minimum_weighted_mean_delta configuration missing; weighted mean delta unproven",
            undefined,
            ["ORACLE-092"],
          );
          ...
          fail(
            "VISUAL_WEIGHTED_MEAN_DELTA_INSUFFICIENT",
            `weighted mean delta ${weightedMean.toFixed(4)} below required ${weightedMeanThreshold}`,
            { weighted_mean_delta: weightedMean, threshold: weightedMeanThreshold },
          );
        }
        ...
        for (const dimension of criticalDimensions ?? []) {
          ...
          if (mean < 0)
            fail("CRITICAL_VISUAL_DIMENSION_REGRESSED", `${dimension} regressed`, { mean });
        }
      }
    }
    ...
    if (blockingInconclusive.length > 0) {
      inconclusive(
        "GOLDEN_ORACLE_BLOCKING_INCONCLUSIVE",
        `${blockingInconclusive.length} blocking inconclusive state(s) prevent GOLDEN_E2E_PASS_IMPROVED`,
        blockingInconclusive.map((state) => state.code),
        ["ORACLE-101"],
      );
    }
...
export function runVerifier(
  casePath,
  receiptPath,
  oraclePath = process.env.SAFEHAVEN_ORACLE_PATH ?? "tests/golden/safehaven/oracle.json",
) {
