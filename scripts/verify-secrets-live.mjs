...
const present = (k) => { const v = val(k); return v !== undefined && !INVALID.has(String(v).trim()); };
const masked = (k) => { const v = val(k); return present(k) ? `set(len=${String(v).length})` : 'MISSING'; };
...
async function http(url, { method = 'GET', headers = {}, body } = {}) {
  ...
  try {
    ...
    return { status: 0, ok: false, err: e.name === 'AbortError' ? 'timeout' : String(e.message || e) };
  } finally {
    clearTimeout(t);
  }
}
...
function tcpProbe(host, port) {
  return new Promise((resolve) => {
    ...
    const done = (r) => { try { sock.destroy(); } catch { /* noop */ } resolve(r); };
...
const results = [];
const required = new Set(['OPENROUTER_API_KEY', 'GITHUB_SITE_TOKEN', 'VERCEL_TOKEN', 'VERCEL_TEAM_ID']);
function record(name, kind, verdict, detail) {
  results.push({ name, kind, present: present(name), value: masked(name), verdict, detail: detail || '' });
}
...
async function githubToken(name) {
  ...
  const r = await http('https://api.github.com/user', {
    headers: { Authorization: `Bearer ${val(name)}`, 'User-Agent': 'l9-secret-doctor', Accept: 'application/vnd.github+json' },
  });
  const verdict = r.status === 200 ? 'valid' : r.status === 401 ? 'invalid(401)' : `http(${r.status})${r.err ? ':' + r.err : ''}`;
...
async function openrouter() {
  ...
  const r = await http('https://openrouter.ai/api/v1/key', { headers: { Authorization: `Bearer ${val(name)}` } });
  record(name, 'llm', r.status === 200 ? 'valid' : r.status === 401 ? 'invalid(401)' : `http(${r.status})${r.err ? ':' + r.err : ''}`);
}
...
async function perplexity() {
  ...
  const verdict = r.status === 200 ? 'valid'
    : r.status === 401 || r.status === 403 ? `invalid(${r.status})`
    : r.status === 400 ? 'auth-ok(400 bad-req)'
...
async function vercel() {
  ...
  if (!present(name)) { record(name, 'vercel', 'missing'); }
  else {
    const r = await http('https://api.vercel.com/v2/user', { headers: { Authorization: `Bearer ${val(name)}` } });
    record(name, 'vercel', r.status === 200 ? 'valid' : r.status === 401 || r.status === 403 ? `invalid(${r.status})` : `http(${r.status})${r.err ? ':' + r.err : ''}`);
    ...
    if (present('VERCEL_TOKEN') && proj) {
      ...
      record('VERCEL_PROJECT_ID', 'vercel', pr.status === 200 ? 'resolves' : `http(${pr.status})`, team ? 'scoped-by-team' : 'no-team-scope');
    }
  }
}
...
async function dataforseo() {
  const l = 'DATAFORSEO_LOGIN', p = 'DATAFORSEO_PASSWORD';
  if (!present(l) || !present(p)) return record(`${l}+${p}`, 'seo', present(l) || present(p) ? 'partial' : 'missing');
  const basic = Buffer.from(`${val(l)}:${val(p)}`).toString('base64');
  const r = await http('https://api.dataforseo.com/v3/appendix/user_data', { headers: { Authorization: `Basic ${basic}` } });
  record(`${l}+${p}`, 'seo', r.status === 200 ? 'valid' : r.status === 401 ? 'invalid(401)' : `http(${r.status})${r.err ? ':' + r.err : ''}`);
}

// --- SEO bot service: best-effort auth probe against its URL ---
async function seoBot() {
  const u = 'SEO_BOT_URL', k = 'SEO_BOT_API_KEY';
  if (!present(u)) return record(`${u}+${k}`, 'seo', 'missing');
  let base = String(val(u)).replace(/\/+$/, '');
  const r = await http(`${base}/health`, { headers: present(k) ? { Authorization: `Bearer ${val(k)}`, 'x-api-key': val(k) } : {} });
  const verdict = r.status === 200 ? 'reachable+ok' : r.status ? `reachable(http ${r.status})` : `unreachable:${r.err}`;
...
async function postgres() {
  ...
  try {
    ...
    record(name, 'db', r.ok ? 'tcp-reachable' : `tcp-fail:${r.err}`, `host=${u.hostname}:${port} (no query run)`);
  } catch {
...
async function inngest() {
  ...
  if (present(name)) {
    const r = await http(`https://inn.gs/e/${encodeURIComponent(val(name))}`, {
      ...
      body: JSON.stringify({ name: 'l9/secret.doctor.probe', data: { source: 'ci-secret-doctor' } }),
    });
    record(name, 'inngest', r.status === 200 ? 'valid(event accepted)' : r.status ? `http(${r.status})` : `err:${r.err}`, 'emitted 1 labeled test event');
  } else record(name, 'inngest', 'missing');
  record('INNGEST_SIGNING_KEY', 'inngest', present('INNGEST_SIGNING_KEY') ? 'present(not live-tested)' : 'missing');
}

async function main() {
  await Promise.all([
    githubToken('GITHUB_SITE_TOKEN'), githubToken('GITHUB_PROVISION_TOKEN'), githubToken('SEO_BOT_SITE_GITHUB_TOKEN'),
    openrouter(), perplexity(), vercel(), dataforseo(), seoBot(), postgres(), inngest(),
  ]);
  ...
  if (present(hook)) { try { hookDetail = `host=${new URL(val(hook)).host} — NOT called`; } catch { hookDetail = 'malformed-url — NOT called'; } }
  record(hook, 'vercel', present(hook) ? 'present(not called)' : 'missing', hookDetail);
  for (const n of ['POSTHOG_KEY', 'PUBLIC_POSTHOG_KEY']) record(n, 'analytics', present(n) ? 'present(not auth-validatable)' : 'missing');
  record('SDK_TOKEN', 'other', present('SDK_TOKEN') ? 'present(no known probe)' : 'missing');

  // Vars (resolution/presence)
  for (const v of ['CLIENT_ID', 'VERCEL_PROJECT_ID', 'VERCEL_TEAM_ID']) record(v, 'var', present(v) ? 'resolves' : 'MISSING');
  const pub = 'CLIENT_PUBLIC_SITE_URL';
  if (present(pub)) { const r = await http(String(val(pub))); record(pub, 'var', r.status ? `http(${r.status})` : `unreachable:${r.err}`); }
  ...
  const summary = {
    ...
    invalid: results.filter((r) => /invalid|tcp-fail|unreachable/.test(String(r.verdict))).map((r) => r.name),
    required_failures: results.filter((r) => required.has(r.name) && (!r.present || /invalid/.test(String(r.verdict)))).map((r) => r.name),
  };

  // Human table
  const pad = (s, n) => String(s).padEnd(n);
  console.log('\n=== Live secret resolution report ===');
  console.log(pad('SECRET/VAR', 34), pad('PRESENT', 8), pad('VALUE', 12), pad('VERDICT', 26), 'DETAIL');
  for (const r of results) console.log(pad(r.name, 34), pad(r.present ? 'yes' : 'NO', 8), pad(r.value, 12), pad(r.verdict, 26), r.detail);
  console.log('\n' + JSON.stringify(summary, null, 2));

  if (STRICT && summary.required_failures.length) { console.error(`\nSTRICT: required secret failures: ${summary.required_failures.join(', ')}`); process.exit(1); }
  process.exit(0);
}
