...
import { appendLearningEvent, loadLearningEvents } from "../src/campaigns/campaign-files.js";
import {
  atomicWriteManifest,
...
async function main(): Promise<void> {
  ...
  const resolvedCampaignId =
    campaignId ?? defaultCampaignId(sourceUrl ?? requiredValue(args, "source"));
  const resolvedSite = campaignId
    ? findSiteForCampaign(baseRoot, campaignId)
    : siteSlugOf(sourceUrl ?? requiredValue(args, "source"));
  const resolvedSourceUrl =
    sourceUrl ??
    loadCampaignManifest(campaignRootOf(baseRoot, resolvedSite, resolvedCampaignId)).source_url;
...
function fileBasedDeps(campaignRoot: string): CampaignDeps {
  ...
  return {
    ...
    async retrieveLearnings(query) {
      ...
      return [
        ...result.confirmed.map((item) => item.event),
        ...result.anti_patterns.map((item) => item.event),
      ];
    },
