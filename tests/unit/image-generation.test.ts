...
import type { ImageGenerationRequest, ImageGenerationResult, ImageGenerator } from '../../src/services/images/ImageGenerator.js';
...
function ogSlot(): ImageSlotSpec {
  return { id: 'og-image', placement: 'global:og-image', required: true, preferredSources: ['generated'], generation: { intent: 'Brand social card' } };
}

function genContext(clientId: string) {
  return fixtureContext({
    client_id: clientId,
    assets: { imageSlots: [ogSlot()], generation: { enabled: true, budgetUsd: 5, model: 'test-gen' } },
  });
}
...
void test('compileImagePrompt is deterministic and honors compiler + exclusions', () => {
  const brief = { slotId: 'og', intent: 'Trustworthy hero photo', composition: 'subject right', exclusions: ['text', 'watermarks'], aspectRatio: '16:9' as const };
...
void test('generation fails closed when the budget is exhausted', async () => {
  const ctx = fixtureContext({
    ...
    assets: { imageSlots: [ogSlot()], generation: { enabled: true, budgetUsd: 0.5, model: 'test-gen' } },
  });
  try {
    await new ImageAssetPlanningStage().run(ctx);
...
void test('generation errors when a gap exists but no provider is configured', async () => {
  ...
  try {
    await new ImageAssetPlanningStage().run(ctx);
    await assert.rejects(() => new ImageGenerationStage().run(ctx), /no image provider is configured/i);
  } finally {
