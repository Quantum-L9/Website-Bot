...
import { rebuildManifestFromLedger, transitionForEvent } from '../../../src/recursive/state/resume.js';
...
test('an interrupted run resumes from the ledger without duplicating semantic operations', () => {
  ...
  try {
    ...
    applyTransition(manifest, { kind: 'HARVEST_COMPLETED', harvestRef: 'h-1', materialActionableSignal: true });
...
test('replay is deterministic: same ledger, same result', () => {
  ...
  try {
    ...
    assert.deepEqual(JSON.parse(JSON.stringify(first.manifest.state)), JSON.parse(JSON.stringify(second.manifest.state)));
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
...
test('every event type maps to exactly one transition action (or a documented no-op)', () => {
  const eventTypes = ['e2e.completed', 'engineering_harvest.completed', 'pe_pack.ready', 'verification.completed', 'pr.merged', 'deployment.succeeded', 'deployment.failed', 'rollback.completed', 'wave.completed'];
