...
} from "../../../src/recursive/signals/registry.js";
...
function signal(overrides: Partial<EngineeringSignal>): EngineeringSignal {
  return {
    ...
    origin: {
      e2eReceiptRef: { refKind: "e2e", refId: "e2e:1", digest: "d".repeat(64) },
      qualityResultRefs: [],
      evidenceRefs: [],
    },
  ...
  const clusters = clusterSignals([
    ...
    signal({
      ...
      causalTrace: {
        upstreamArtifactsInspected: [],
        unchangedControls: [],
        suspectedOwner: { repository: "Quantum-L9/Website-Bot", subsystem: "DesignIntelligence" },
      },
    }),
  ]);
  ...
  const clusters = clusterSignals([
    ...
    signal({
      ...
      causalTrace: {
        upstreamArtifactsInspected: [],
        unchangedControls: [],
        suspectedOwner: { repository: "Quantum-L9/Website-Bot", subsystem: "Other" },
      },
      leverage: {
        humanReviewImpact: "LOW",
        downstreamCostImpact: "LOW",
        recurrence: "LOW",
...
test("low-confidence clusters are never eligible for a PE pack", () => {
  const clusters = clusterSignals([signal({ signalId: "ES-1", confidence: "LOW" })]);
  ...
  const clusters = clusterSignals([
    signal({
      ...
      strongestAlternative: {
        statement: "blueprint defect",
        confidence: "MEDIUM",
        disconfirmingTest: "inspect blueprint",
        result: "INCONCLUSIVE",
      },
    }),
  ]);
