...
test("low-confidence and control-plane clusters are rejected by the compiler", () => {
  ...
  assert.throws(
    () => compilePEPack({ ...base, cluster: { ...cluster, confidence: "LOW" } }),
    /low-confidence/,
  );
  assert.throws(
    () =>
      compilePEPack({
        ...base,
        cluster: { ...cluster, signalClass: "CONTROL_PLANE_CHANGE_REQUIRED" },
      }),
    /control-plane/,
  );
});
...
test("the frozen acceptance contract is digest-bound before mutation", () => {
  ...
  assert.equal(
    compiled.frozenContract.forbiddenChanges.some((change) => change.property === "control plane"),
    true,
  );
});
