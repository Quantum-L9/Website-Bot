...
import { createCampaignManifest, isLegalWave, validateCampaignManifest } from '../../../src/recursive/state/run-manifest.js';
...
function driveWave(manifest: ReturnType<typeof fixture>): void {
  const ok = (applied: boolean, reason?: string) => assert.equal(applied, true, reason ?? 'transition refused');
  ok(applyTransition(manifest, { kind: 'E2E_COMPLETED', reviewable: false, e2eReceiptRef: 'e2e-1', deployedSha: 'a'.repeat(40) }).applied);
  ok(applyTransition(manifest, { kind: 'HARVEST_COMPLETED', harvestRef: 'harvest-1', materialActionableSignal: true }).applied);
  ok(applyTransition(manifest, { kind: 'PE_PACK_COMPILED', pePackRef: 'pack-1', clusterId: 'EC-1' }).applied);
...
test('reviewable E2E with no material signal stops the run early', () => {
  const manifest = fixture();
  assert.equal(applyTransition(manifest, { kind: 'E2E_COMPLETED', reviewable: true, e2eReceiptRef: 'e2e-1', deployedSha: 'a'.repeat(40) }).applied, true);
...
test('non-reviewable E2E with no material signal stops as NO_ACTIONABLE_SIGNAL', () => {
  const manifest = fixture();
  applyTransition(manifest, { kind: 'E2E_COMPLETED', reviewable: false, e2eReceiptRef: 'e2e-1', deployedSha: 'a'.repeat(40) });
  const result = applyTransition(manifest, { kind: 'HARVEST_COMPLETED', harvestRef: 'h', materialActionableSignal: false });
...
test('patch validation failure and deployment failure are terminal', () => {
  ...
  applyTransition(patchFail, { kind: 'E2E_COMPLETED', reviewable: false, e2eReceiptRef: 'e', deployedSha: 'a'.repeat(40) });
  applyTransition(patchFail, { kind: 'HARVEST_COMPLETED', harvestRef: 'h', materialActionableSignal: true });
  ...
  applyTransition(deployFail, { kind: 'E2E_COMPLETED', reviewable: false, e2eReceiptRef: 'e', deployedSha: 'a'.repeat(40) });
  applyTransition(deployFail, { kind: 'HARVEST_COMPLETED', harvestRef: 'h', materialActionableSignal: true });
...
test('scope-insufficient verdict is terminal without broadening', () => {
  ...
  applyTransition(manifest, { kind: 'HARVEST_COMPLETED', harvestRef: 'h', materialActionableSignal: true });
