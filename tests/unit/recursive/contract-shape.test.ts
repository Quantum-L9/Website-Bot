...
test("emitted schemas validate against the repo schema compiler with zero drift", () => {
  const result = spawnSync(process.execPath, ["scripts/validate-recursive-schemas.mjs"], {
    encoding: "utf-8",
  });
...
function samplePack(): PEPack {
  return {
    ...
    hypothesis: {
      proposedSystemChange: "change",
      expectedSystemEffect: "effect",
      expectedE2EEffect: "e2e effect",
    },
    mutationEnvelope: {
      repository: "Quantum-L9/Website-Bot",
      allowedPaths: [],
...
function sampleOutcome(): CodeChangeOutcome {
  ...
  return {
    ...
    code: {
      repository: "Quantum-L9/Website-Bot",
      beforeFullSha: "a".repeat(40),
      patchedFullSha: "b".repeat(40),
    },
...
test("bound contract field sets are pinned exactly as the pack requires", () => {
  ...
  for (const [schemaId, sample] of Object.entries(samples) as Array<
    [keyof typeof pinned, unknown]
  >) {
    ...
    for (const key of keys) {
      assert.ok(
        ...
        `${schemaId} drifted: unbound field ${key}`,
      );
    }
  }
});
