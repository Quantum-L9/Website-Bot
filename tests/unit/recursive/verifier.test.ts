...
function verifierInput(overrides: Partial<Parameters<IndependentVerifier['verify']>[0]> = {}) {
  return {
    ...
    originating: [
      {
        caseRef: { caseId: 'REG-1', ref: { refKind: 'regression', refId: 'REG-1', digest: sha256Text('REG-1') } },
        beforeResult: 'FAIL',
        afterResult: 'PASS',
    ...
    controls: [
      {
        caseRef: { caseId: 'CTRL-1', ref: { refKind: 'control', refId: 'CTRL-1', digest: sha256Text('CTRL-1') } },
        beforeResult: 'PASS',
        afterResult: 'PASS',
...
test('visible cases passing while holdout fails blocks with FAIL_HOLDOUT', () => {
  const verifier = new IndependentVerifier('independent-verifier');
  const receipt = verifier.verify(verifierInput({ holdoutCases: [{ caseId: 'HOLD-1', passed: false }] }));
...
test('repository check failure blocks with FAIL_CI', () => {
  const verifier = new IndependentVerifier('independent-verifier');
  const receipt = verifier.verify(
    verifierInput({ repositoryChecks: [{ name: 'typecheck', passed: false }, { name: 'unit', passed: true }] }),
  );
...
test('envelope violation blocks with FAIL_SCOPE', () => {
  const verifier = new IndependentVerifier('independent-verifier');
  const receipt = verifier.verify(verifierInput({ changedFiles: ['src/recursive/state/constants.ts'] }));
