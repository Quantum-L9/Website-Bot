...
import { DeploymentVerifier, LocalDirectoryDeploymentAdapter } from '../../../src/recursive/deployment/verifier.js';
...
test('successful deployment returns PASS with the exact deployed SHA', async () => {
  ...
  try {
    const verifier = new DeploymentVerifier(new LocalDirectoryDeploymentAdapter(join(root, 'target'), join(root, 'health.txt')));
...
test('deployment SHA provenance mismatch fails closed and rolls back', async () => {
  ...
  try {
    ...
    const verifier = new DeploymentVerifier(new MismatchingAdapter(join(root, 'target'), join(root, 'health.txt')));
    const { receipt, rolledBack } = await verifier.deployAndVerify({
      mergeSha: MERGE_SHA,
      environment: 'preview',
      previousVerifiedSha: PREVIOUS_SHA,
...
test('health failure triggers automatic rollback to the previous verified SHA', async () => {
  ...
  try {
    ...
    const verifier = new DeploymentVerifier(new UnhealthyAdapter(join(root, 'target'), join(root, 'health.txt')));
    const { receipt, rolledBack } = await verifier.deployAndVerify({
      mergeSha: MERGE_SHA,
      environment: 'preview',
      previousVerifiedSha: PREVIOUS_SHA,
...
test('deployment attempts are budgeted: the final attempt rolls back and fails', async () => {
  ...
  try {
    ...
    const verifier = new DeploymentVerifier(new CountingFailingAdapter(join(root, 'target'), join(root, 'health.txt')));
    const { receipt, rolledBack } = await verifier.deployAndVerify({
      mergeSha: MERGE_SHA,
      environment: 'preview',
      previousVerifiedSha: PREVIOUS_SHA,
...
test('rollback idempotency: rollback receipt carries a stable digest', async () => {
  const root = mkdtempSync(join(tmpdir(), 'deploy-'));
  try {
    const adapter = new LocalDirectoryDeploymentAdapter(join(root, 'target'), join(root, 'health.txt'));
