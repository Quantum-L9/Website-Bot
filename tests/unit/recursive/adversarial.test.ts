...
} from "../../../src/recursive/contracts/validate.js";
...
test("duplicate hook becomes a NOOP", () => {
  ...
  try {
    ...
    assert.equal(
      ledger.ingest(signEvent(envelope.event, "adv-secret"), "adv-secret", 1).disposition,
      "DUPLICATE",
    );
...
test("coder self-certification attempt fails closed", () => {
  ...
  assert.throws(() => verifier.verify({ pack: selfCertified } as never), /identity mismatch/);
});
...
test("attempted wave four is hard rejected and unrepresentable", () => {
  ...
  assert.equal(
    manifest.state.phases.every((phase) => phase.wave <= 3),
    true,
  );
