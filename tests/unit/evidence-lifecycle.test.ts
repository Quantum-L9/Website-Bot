...
test("persisted artifacts use snake_case and concurrent writers leave a verifiable index", async () => {
  ...
  const files = [
    {
      ...
      sha256: "a".repeat(64),
      owner: "website-bot" as const,
      bytes: 1,
    },
  ];
  const sourceDigest = (
    await import("../../src/pipeline/evidence/AssemblyManifest.js")
  ).computeAssemblySourceDigest(files);
  const base = {
  ...
  const raw = JSON.parse(
    (await import("node:fs")).readFileSync(join(rootDir, "assembly-manifest.json"), "utf8"),
