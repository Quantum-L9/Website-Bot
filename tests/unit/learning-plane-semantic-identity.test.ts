...
test('digest is time-independent: produced_at does not change it', () => {
  ...
  const second = payloadDigestOf({ ...base, payload: { value: 42, produced_at: '2026-08-15T00:00:00Z' } });
  assert.notEqual(first, second, 'payload differences change the digest; identity is over the semantic body');
});

test('semantic_input_digest covers stage version, refs, and configuration', () => {
  const ref = { artifact_type: 'PageContentContract', artifact_id: 'PageContentContract:pcc1', payload_digest: 'pcc1' };
  const base = { stage_version: '2.0.0', relevant_input_artifact_refs: [ref], relevant_configuration: { tone: 'trust-first' } };
  const digest = semanticInputDigest(base);
  assert.equal(digest.length, 64);
  assert.equal(semanticInputDigest(base), digest);
  assert.notEqual(semanticInputDigest({ ...base, relevant_configuration: { tone: 'proof-first' } }), digest);
});
