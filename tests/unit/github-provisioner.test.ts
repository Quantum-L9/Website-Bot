...
} from "../../src/provisioning/GitHubProvisioner.js";
...
function request(): ProvisioningRequest {
  return {
    ...
    github: {
      ...
      visibility: "private",
      description: "Acme generated site",
      sourceBranch: "main",
...
void test("adopts an existing repository without creating or deleting it", async () => {
  ...
  const fetchImpl: FetchLike = async (input, init) => {
    ...
    if (url.endsWith("/repos/Quantum-L9/acme-site"))
      return json(200, {
        id: 42,
        full_name: "Quantum-L9/acme-site",
        html_url: "https://github.com/Quantum-L9/acme-site",
        default_branch: "main",
      });
...
void test("creates a missing organization repository and verifies its initialized branch", async () => {
  ...
  const fetchImpl: FetchLike = async (input, init) => {
    ...
    if (url.endsWith("/repos/Quantum-L9/acme-site") && method === "GET")
      return json(404, { message: "Not Found" });
    if (url.endsWith("/users/Quantum-L9"))
      return json(200, { login: "Quantum-L9", type: "Organization" });
...
void test("exposes a created repository for compensation when branch convergence fails", async () => {
  const fetchImpl: FetchLike = async (input, init) => {
    ...
    if (url.endsWith("/users/Quantum-L9"))
      return json(200, { login: "Quantum-L9", type: "Organization" });
    if (url.endsWith("/orgs/Quantum-L9/repos"))
      return json(201, {
        id: 44,
        full_name: "Quantum-L9/acme-site",
        html_url: "https://github.com/Quantum-L9/acme-site",
