...
test('reviewable predicate rejects on hard-gate failure regardless of other verdicts', () => {
  const index = buildQualityDeltaIndex({
    ...
    candidate_id: 'C1',
    results: [
      ...reviewableIndex().results.filter(result => result.dimension !== 'business.fact_accuracy'),
...
test('repeated evaluation yields the same semantic identity', () => {
  ...
  assert.equal(
    payloadDigestOf({ protocol: 'l9', protocol_version: '1', artifact_type: 'CandidateEvaluation', input_refs: [], payload: first }),
    payloadDigestOf({ protocol: 'l9', protocol_version: '1', artifact_type: 'CandidateEvaluation', input_refs: [], payload: second }),
  );
});
