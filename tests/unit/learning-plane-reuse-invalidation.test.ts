...
const REF = (id: string) => ({
  artifact_type: "WebsiteBuildBlueprint",
  artifact_id: `WebsiteBuildBlueprint:${id}`,
  payload_digest: id,
});
...
test("a blueprint mutation invalidates downstream content and design", () => {
  ...
  for (const stage of [
    ...
    assert.ok(
      (frontier.invalidate as readonly string[]).includes(stage),
      `BLUEPRINT must invalidate ${stage}`,
    );
  }
  for (const stage of ["donor-intelligence", "dataforseo", "pattern-synthesis"]) {
    assert.ok(
      (frontier.reuse as readonly string[]).includes(stage),
      `BLUEPRINT must reuse ${stage}`,
    );
  }
});
...
test("completed content-addressed artifacts are not recomputed: same digest reuses", () => {
  const inputs = {
    stage_version: "1.3.0",
    relevant_input_artifact_refs: [REF("aaa"), REF("bbb")],
    relevant_configuration: { mode: "design" },
  };
  const first = semanticInputDigest(inputs);
  const second = semanticInputDigest({
    ...inputs,
    relevant_input_artifact_refs: [REF("bbb"), REF("aaa")],
  });
