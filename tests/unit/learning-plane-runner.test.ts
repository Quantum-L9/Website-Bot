...
function indexFor(
  candidate: string,
  entries: Array<[string, "IMPROVED" | "REGRESSED" | "NON_REGRESSED" | null, ("PASS" | "FAIL")?]>,
): QualityDeltaIndex {
  return buildQualityDeltaIndex({
    campaign_id: "fixture-001",
    candidate_id: candidate,
    results: entries.map(([dimensionName, verdict, status]) =>
      dimension(candidate, dimensionName, verdict, status ?? "PASS"),
    ),
  });
}
      ...
      inherited_exact: [
        {
          ...
          artifact_id: "WebsiteBuildBlueprint:wb1",
          payload_digest: "wb1",
        },
      ],
...
const GOOD_C0: Array<
  [string, "IMPROVED" | "REGRESSED" | "NON_REGRESSED" | null, ("PASS" | "FAIL")?]
> = [
...
test("runner exhausts when the budget is consumed without convergence", async () => {
  ...
  const outcome = await runCampaign({
    ...
    deps: memoryDeps({
      ...
      C1: indexFor("C1", NON_REVIEWABLE_C0),
    }),
  });
