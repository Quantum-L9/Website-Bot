...
test('a torn manifest is never loadable as valid state', () => {
  const root = tempCampaignRoot();
  writeFileSync(join(root, 'campaign-manifest.json'), '{"schema":"website-bot.campaign-manifest/v1","truncated":', 'utf8');
...
test('campaign resumes from persisted state after process death', () => {
  ...
  manifest = updateCampaignManifest(loadCampaignManifest(root), {
    champion: {
      candidate_id: 'C2',
      build_ref: { artifact_type: 'CandidateBuild', artifact_id: 'CandidateBuild:C2', payload_digest: 'd' },
      evaluation_ref: { artifact_type: 'CandidateEvaluation', artifact_id: 'CandidateEvaluation:C2', payload_digest: 'd' },
    },
    attempts: { total_candidates: 3, no_progress_rounds: 0, blueprint_replans: 0, content_regenerations: 0, repairs_by_candidate: {} },
  });
  atomicWriteManifest(root, manifest);
