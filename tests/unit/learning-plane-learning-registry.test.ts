...
test('negative learning is first-class and retrievable', () => {
  const rejected = event({ outcome: 'REJECTED', anti_pattern: { invariant: 'second CTA regressed hierarchy' } });
...
test('promotion confidence is evidence-weighted and deterministic', () => {
  ...
  const high = buildPromotionCandidate({ ...base, wins: 5, losses: 0, human_approved_campaigns: 1 });
