...
interface SpawnCall { command: string; args: string[]; options: { cwd: string; env: NodeJS.ProcessEnv; shell: boolean; stdio: unknown }; }
...
function makeSpawn() {
  ...
  child.kill = (signal = 'SIGTERM') => { kills.push(signal); return true; };
...
void test('spawns with shell:false and the ignore/pipe/pipe stdio contract', async () => {
  ...
  const p = new SpawnCommandRunner(spawnImpl).run('npm', ['ci', '--no-audit'], { cwd: '/work', timeoutMs: 1_000, env: ENV });
...
void test('resolves with captured stdout, stderr, and a duration on a zero exit', async () => {
  ...
  const p = new SpawnCommandRunner(spawnImpl).run('cmd', [], { cwd: '/work', timeoutMs: 1_000, env: ENV });
...
void test('times out with SIGTERM and escalates to SIGKILL after the grace period', async (t) => {
  ...
  const p = new SpawnCommandRunner(spawnImpl).run('sleep', [], { cwd: '/work', timeoutMs: 1_000, env: ENV });
  ...
  await assert.rejects(p, (error: unknown) =>
    error instanceof BuildError && error.code === 'BUILD_FAILED' && /signal=SIGKILL/.test(error.message));
});
...
void test('clears the timeout timer when the child fails to start', async (t) => {
  ...
  await assert.rejects(p, (error: unknown) =>
    error instanceof BuildError && error.code === 'BUILD_FAILED' && /failed to start/.test(error.message));
...
void test('real subprocess: an overrunning command is terminated with SIGTERM', async () => {
  ...
  await assert.rejects(
    () => runner.run(
      process.execPath,
      ['-e', 'setTimeout(() => {}, 60000)'],
      { cwd: process.cwd(), timeoutMs: 200, env: process.env },
    ),
    (error: unknown) => error instanceof BuildError && error.code === 'BUILD_FAILED' && /signal=SIGTERM/.test(error.message),
  );
});
...
void test('real subprocess: a missing executable rejects as a start failure', async () => {
  const runner = new SpawnCommandRunner();
  await assert.rejects(
    () => runner.run('this-command-does-not-exist-website-bot', [], { cwd: process.cwd(), timeoutMs: 5_000, env: process.env }),
    (error: unknown) => error instanceof BuildError && error.code === 'BUILD_FAILED' && /failed to start/.test(error.message),
  );
});
