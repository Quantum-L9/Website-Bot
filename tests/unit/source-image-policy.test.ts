...
import {
  ...
  normalizeUrl,
  shouldSkipUrl,
} from "../../src/ingestion/SourceImagePolicy.js";
...
void test("accepts a compact logo that would fail the photo minimum", () => {
  assert.equal(
    evaluateSourceImage(inspected({ width: 298, height: 144 }), undefined, {
      sourceUrl: "https://x.test/images/logo.webp",
    }).accepted,
    true,
  );
});
...
void test("accepts SVG without intrinsic dimensions", () => {
  assert.equal(
    evaluateSourceImage(inspected({ mimeType: "image/svg+xml", width: 0, height: 0 })).accepted,
    true,
  );
});
