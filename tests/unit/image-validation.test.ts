...
async function assembleWithProvidedHero(clientId: string, sourcePath: string) {
  const ctx = fixtureContext({
    ...
    assets: {
      imageSlots: [heroSlot],
      providedImages: [
        { id: "hero", path: sourcePath, intendedPlacement: "/:hero", altText: "Hero" },
      ],
    },
  });
...
void test("passes when every referenced image exists on disk", async () => {
  ...
  try {
    ...
    assert.equal(
      ctx.imageProvenanceWarnings,
      undefined,
      "provided images carry no provenance warnings",
    );
  } finally {
    cleanupContext(ctx);
...
void test("surfaces source-site provenance warnings for release evidence", async () => {
  ...
  const manifest: SourceSiteManifest = {
    ...
    images: [
      {
        id: "src-hero",
        sourceUrl: "https://acme.example/hero.png",
