...
import { buildCrawlIdentity, overlayCrawlIdentity, routesFromCrawl } from '../../src/services/spec/crawlIdentity.js';
...
const manifest: SourceSiteManifest = {
  ...
  rejected: [],
  warnings: [],
  palette: { primary: '#1ca0e0', secondary: '#171717', accent: '#1ca0e0', background: '#000000', text: '#ffffff' },
};

void test('routes come from crawled slugs, not an invented service list', () => {
  const routes = routesFromCrawl(manifest.pages);
  assert.deepEqual(routes.map(route => route.slug), ['/', '/gallery', '/services/roof-repair']);
...
void test('CODE identity wins over an LLM-invented spec', () => {
  ...
  assert.ok(routes.every(route => ['/', '/gallery', '/services/roof-repair'].includes(route.slug)));
