...
void test("emits handoff only from a succeeded persisted release bundle", async () => {
  ...
  try {
    ...
    await new HandoffEmitterStage(
      fetch,
      join(outputRoot, "handoff.yaml"),
      join(outputRoot, "ack.json"),
    ).run(ctx);
    const stored = await ctx.evidenceStore.readHandoff();
    assert.ok(stored);
    assert.equal(stored?.site.repository.commit_sha, publication.commitSha);
    assert.equal(
      stored?.proof.receipt_id,
      (await ctx.evidenceStore.readReleaseReceipt())?.value.receipt_id,
    );
    assert.ok(existsSync(join(outputRoot, "handoff.yaml")));
    const yaml = parse(readFileSync(join(outputRoot, "handoff.yaml"), "utf-8")) as {
      contract_id?: string;
    };
  ...
  try {
    ...
    const fakeFetch = async (
      ...
      return Response.json({
        ...
        verified_repository: contract.site.repository.full_name,
        verified_branch: contract.site.repository.branch,
        verified_commit_sha: "f".repeat(40),
        probes: [
          { name: "a", ok: true },
          { name: "b", ok: true },
          { name: "c", ok: true },
