...
const CLEAN_COPY =
  "Safe Haven Roofing serves Charlotte homeowners with storm damage inspections, " +
...
test("scanText: legitimate bracketed citations do not trip the bracket pattern", () => {
  assert.deepEqual(
    scanText("content:/:hero", "Rated top roofer [2025] by the Charlotte Register."),
    [],
  );
});
...
test("stage: fails closed with PLACEHOLDER_CONTENT_DETECTED and full finding context", async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => {
      ...
      const findings = (error.context?.findings ?? []) as Array<{
        source: string;
        patternId: string;
      }>;
