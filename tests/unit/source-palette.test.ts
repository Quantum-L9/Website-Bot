...
void test('gallery URLs are crawled before generic service pages', () => {
  assert.ok(crawlPagePriority('https://x.test/gallery') < crawlPagePriority('https://x.test/services/roof-repair'));
...
void test('design stage prefers crawled palette over an LLM-resolved spec', async () => {
  const ctx = fixtureContext({
    design: { status: 'resolved', palette: { primary: '#2C5530', secondary: '#8B6914' }, fonts: { heading: 'Inter', body: 'Inter' } },
  });
  ctx.sourceSiteManifest = {
    ...
    rejected: [],
    warnings: [],
    palette: { primary: '#1ca0e0', secondary: '#171717', accent: '#1ca0e0', background: '#0a0a0a', text: '#f5f5f5' },
  };
  try {
