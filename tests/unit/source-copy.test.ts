...
const manifest: SourceSiteManifest = {
  ...
  pages: [{
    ...
    bodyText: 'We repair and replace roofs across the Charlotte metro. Storm damage, insurance claims, and new construction.',
    phones: ['(704) 648-7252'],
    nav: [
...
void test('top nav keeps depth-1 items and drops mega-menu children', () => {
  ...
  assert.deepEqual(nav.map(item => item.href), ['/services', '/gallery', '/about', '/contact']);
});
