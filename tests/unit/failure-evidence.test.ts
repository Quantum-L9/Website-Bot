...
void test("sanitizes provider failures and persists failed evidence/checkpoint state", async () => {
  ...
  try {
    const message = sanitizeEvidenceText(
      "provider rejected token ghp_ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890",
    );
    const details = sanitizeEvidenceDetails({
    ...
    assert.equal(
      (await ctx.evidenceStore.readCheckpoint("credential-bound-stage"))?.status,
      "failed",
    );
