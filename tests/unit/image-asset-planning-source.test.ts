...
import type {
  IngestedImage,
  SourceSiteManifest,
} from "../../src/pipeline/evidence/SourceSiteManifest.js";
import {
  planImageAssets,
  routeSlugFromPlacement,
...
function sourceImage(
  ...
  return {
    id,
    sourceUrl: `https://acme.example${page}${id}.jpg`,
    referringPageUrl: `https://acme.example${page}`,
    localPath,
    altText: id,
    mimeType: "image/png",
    width,
...
void test("planner assigns the real wordmark to the logo slot, not the OG card", () => {
  ...
  const slots: ImageSlotSpec[] = [
    { id: "logo", placement: "global:logo", required: true, preferredSources: ["source-site"] },
    {
      ...
      required: true,
      aspectRatio: "16:9",
      preferredSources: ["source-site"],
    },
    {
      id: "svc-hero",
      placement: "/services/roof-replacement:hero",
      required: true,
      aspectRatio: "16:9",
      preferredSources: ["source-site"],
    },
  ...
  const ids = plan.assets.map((asset) =>
    asset.resolution.source === "source-site" ? asset.resolution.candidateId : undefined,
  );
  assert.equal(new Set(ids).size, ids.length);
  assert.equal(
    plan.assets[0].resolution.source === "source-site" && plan.assets[0].resolution.candidateId,
    "wordmark",
  );
  assert.notEqual(
    plan.assets[1].resolution.source === "source-site" && plan.assets[1].resolution.candidateId,
    "og-card",
  );
  assert.notEqual(
    plan.assets[1].resolution.source === "source-site" && plan.assets[1].resolution.candidateId,
...
void test("planner prefers a source candidate found on the slot’s own route", () => {
  ...
  const plan = planImageAssets({
    slots: [slot],
    provided: [],
    sourceCandidates: [homeHero, servicesHero],
    generationEnabled: false,
  });
...
void test("stage resolves a crawled source-site image into the build with provenance", async () => {
  ...
  try {
    ...
    assert.ok(
      (resolved?.provenanceWarnings.length ?? 0) > 0,
      "source-site reuse carries a provenance warning",
    );
