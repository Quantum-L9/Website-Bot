...
} from "../../src/provisioning/VercelProvisioner.js";
...
function request(): ProvisioningRequest {
  return {
    ...
    github: {
      ...
      description: "Acme",
      sourceBranch: "main",
      publishCredentialRef: "env://GITHUB_SITE_TOKEN",
    },
    vercel: {
      project: "acme-site",
      environment: [
...
void test("creates and links an Astro project, then upserts declared environment keys", async () => {
  ...
  const fetchImpl: FetchLike = async (input, init) => {
    ...
    if (url.includes("/v9/projects/acme-site") && method === "GET")
    ...
    if (url.includes("/v10/projects/prj_1/env?upsert=true") && method === "POST")
      return json(201, { created: [], failed: [] });
    return json(500, { error: { message: `unexpected ${method} ${url}` } });
  };
  try {
    const result = await new VercelProvisioner(fetchImpl).provision(
      request(),
      repository,
      "vercel-token",
    );
    ...
    assert.deepEqual(JSON.parse(create?.body ?? "{}").gitRepository, {
      type: "github",
      repo: "Quantum-L9/acme-site",
    });
...
void test("authenticates and pins the Astro build contract on project creation and env upsert", async () => {
  ...
  try {
    ...
    const envBody = JSON.parse(String(envInit?.body)) as Array<{
      key: string;
      type: string;
      target: string[];
    }>;
...
void test("rejects an existing project linked to another repository", async () => {
  ...
  await assert.rejects(
    new VercelProvisioner(fetchImpl).provision(
      { ...request(), vercel: { project: "acme-site", environment: [] } },
      repository,
      "token",
    ),
...
void test("exposes a newly created project for compensation when environment convergence fails", async () => {
  ...
  try {
    await assert.rejects(
      ...
      (error: unknown) =>
        error instanceof VercelProvisioningError && error.result?.created === true,
    );
  } finally {
    delete process.env.ACME_SITE_URL;
  }
