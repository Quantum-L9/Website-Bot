...
async function prepareFullChain() {
  ...
  ctx.evidenceStore = new FileEvidenceStore({
    rootDir: `${ctx.outputDir}.chain-evidence`,
...
void test("validates a complete persisted end-to-end evidence chain", async () => {
  ...
  try {
    ...
    assert.equal(
      bundle.validation.identities.deploymentId,
      bundle.deploymentEvidence?.deploymentId,
    );
  } finally {
    cleanupContext(ctx);
  }
...
void test("rejects a deployment artifact that is valid alone but bound to another published commit", async () => {
  ...
  try {
    ...
    ctx.mode = "end-to-end";
    ctx.evidenceStore = new FileEvidenceStore({
      rootDir: `${ctx.outputDir}.mismatch-evidence`,
      clientId: ctx.clientId,
      buildId: ctx.buildId,
      mode: "end-to-end",
    });
    ...
    assert.ok(
      validation.errors.some((error) =>
        error.includes("deployment commit differs from published commit"),
      ),
    );
  } finally {
    cleanupContext(ctx);
