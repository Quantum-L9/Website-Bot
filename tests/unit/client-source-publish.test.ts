...
async function prepareContext() {
  const ctx = fixtureContext({
    deploy: { github_repo: "example/disposable-site", source_branch: "main" },
  });
  ...
  try {
    ...
    const fakeFetch = async (
      ...
      if (url.includes("/git/ref/heads/")) {
        refReads++;
        return Response.json({ object: { sha: gitSha("a") } });
      }
      if (url.includes(`/git/commits/${gitSha("a")}`))
        return Response.json({ tree: { sha: gitSha("b") } });
      if (url.includes(`/git/trees/${gitSha("b")}?recursive=1`))
        return Response.json({ sha: gitSha("b"), truncated: false, tree: [] });
...
void test("refuses publication when source changed after persisted build proof", async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => new ClientSourcePublishStage().run(ctx),
      /changed after local proof/,
    );
  ...
  try {
    ...
    const fakeFetch = async (input: string | URL | Request): Promise<Response> => {
      ...
      if (url.includes(`/git/trees/${gitSha("b")}?recursive=1`))
        return Response.json({ sha: gitSha("b"), truncated: false, tree: [] });
      if (url.includes("/contents/.l9/generated-manifest.json"))
        return Response.json({ encoding: "base64", content: previousManifest });
      throw new Error(`Unexpected GitHub request ${url}`);
    };
    await withEnv({ GITHUB_SITE_TOKEN: "test-token" }, async () => {
      await assert.rejects(
        () => new ClientSourcePublishStage(fakeFetch).run(ctx),
        /refusing cross-client overwrite/,
      );
    });
  } finally {
    cleanupContext(ctx);
...
void test("refuses non-fast-forward publication when branch head moves before update", async () => {
  ...
  try {
    ...
    await withEnv({ GITHUB_SITE_TOKEN: "test-token" }, async () => {
      await assert.rejects(
        () =>
          new ClientSourcePublishStage(
            fakeFetch,
            () => new Date(),
            async () => {},
          ).run(ctx),
        /Target branch changed during publication/,
      );
    });
...
void test("records a no-op when the generated repository tree already matches", async () => {
  ...
  try {
    ...
    tree.push({
      ...
      type: "blob",
      mode: "100644",
      sha: gitBlobSha(Buffer.from(manifestText, "utf-8")),
    });
    ...
    await withEnv({ GITHUB_SITE_TOKEN: "test-token" }, async () => {
      await new ClientSourcePublishStage(fakeFetch, () => new Date("2026-07-20T00:00:02.000Z")).run(
        ctx,
      );
    });
    const stored = await ctx.evidenceStore.readPublication();
  ...
  try {
    ...
    const fakeFetch = async (
      ...
      if (url.endsWith("/git/trees")) {
        submittedTree = (
          JSON.parse(String(init.body)) as { tree: Array<{ path: string; sha: string | null }> }
        ).tree;
