...
void test("deploys an explicitly disposable Vercel project and correlates its commit", {
  skip:
    !process.env.WEBSITE_BOT_TEST_VERCEL_PROJECT_ID ||
    !process.env.WEBSITE_BOT_TEST_GITHUB_REPO_ID ||
    !process.env.WEBSITE_BOT_TEST_COMMIT_SHA ||
    !process.env.VERCEL_TOKEN,
}, async () => {
  const projectId = process.env.WEBSITE_BOT_TEST_VERCEL_PROJECT_ID as string;
  assert.match(
    projectId,
    /(?:test|disposable|throwaway)/i,
    "refusing to mutate a project not marked disposable/test",
  );
