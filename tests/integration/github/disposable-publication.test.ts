...
void test("publishes to an explicitly disposable GitHub repository", {
  skip: !process.env.WEBSITE_BOT_TEST_GITHUB_REPO || !process.env.GITHUB_SITE_TOKEN,
}, async () => {
  const repository = process.env.WEBSITE_BOT_TEST_GITHUB_REPO as string;
  assert.match(
    repository,
    /(?:disposable|throwaway|test)/i,
    "refusing to mutate a repository not marked disposable/test",
  );
  const ctx = fixtureContext({
    deploy: {
      github_repo: repository,
      source_branch: process.env.WEBSITE_BOT_TEST_GITHUB_BRANCH ?? "website-bot-e2e",
    },
  });
  try {
