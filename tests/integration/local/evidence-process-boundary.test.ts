...
void test("a fresh process-equivalent store rehydrates the succeeded release bundle", async () => {
  ...
  try {
    ...
    const rehydrated = new FileEvidenceStore({
      rootDir,
      clientId: ctx.clientId,
      buildId: ctx.buildId,
      mode: "end-to-end",
    });
    const bundle = await rehydrated.loadValidatedReleaseBundle({
    ...
    assert.equal(
      bundle.publicationEvidence?.commitSha,
      bundle.deploymentEvidence?.observedCommitSha,
    );
  } finally {
    cleanupContext(ctx);
  }
