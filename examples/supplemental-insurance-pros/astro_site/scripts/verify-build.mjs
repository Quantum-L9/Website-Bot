...
if (!hasNodeModules) {
  rows.push(
    result(
      ...
      "Astro dependency installed before build execution",
      "node_modules missing in current environment",
      "BLOCKED",
      "critical",
      "Run npm install or npm ci, then rerun npm run verify:build.",
    ),
  );
} else {
  ...
  rows.push(
    result(
      ...
      exists("dist") ? "exists" : "missing",
      exists("dist") ? "PASS" : "FAIL",
      "critical",
      "Fix Astro build output.",
    ),
  );
}
