...
rows.push(
  result(
    ...
    "crm_validation",
    ".env.example",
    "CRM provider declared or Unknown labeled",
    provider,
    provider === "acculynx" ? "PASS" : "UNKNOWN",
    "medium",
    "Confirm CRM provider.",
  ),
  result(
    "CRM-PHASE",
    "crm_validation",
    ".env.example",
    "CRM sync remains phase_2 until configured",
    mode,
    mode === "phase_2" ? "PASS" : "FAIL",
    "high",
    "Set PUBLIC_CRM_SYNC_MODE=phase_2 unless live sync is implemented.",
  ),
  result(
    "CRM-ENDPOINT",
    "crm_validation",
    ".env.example",
    "CRM endpoint not hardcoded and Unknown labeled if absent",
    endpoint.includes("UNKNOWN") ? "UNKNOWN_DECLARED" : "CONFIGURED",
    endpoint.includes("UNKNOWN") ? "UNKNOWN" : "PASS",
    "high",
