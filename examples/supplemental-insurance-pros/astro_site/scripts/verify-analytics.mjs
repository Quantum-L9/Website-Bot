...
rows.push(
  result(
    ...
    "src/scripts/attribution.js",
    "UTM/source attribution persistence exists",
    hasAttribution ? "present" : "missing",
    hasAttribution ? "PASS" : "FAIL",
    "high",
    "Add attribution persistence script.",
  ),
);
...
rows.push(
  result(
    ...
    "analytics_validation",
    ".env.example",
    "analytics config declared",
    configured ? `${provider} configured` : `provider=${provider}, id=UNKNOWN`,
    configured ? "PASS" : "UNKNOWN",
    "medium",
...
rows.push(
  result(
    "ANALYTICS-CONVERSION-MARKER",
    "analytics_validation",
    "LeadForm/BaseLayout",
    "conversion tracking hook exists",
    conversionMarker ? "present" : "missing",
    conversionMarker ? "PASS" : "FAIL",
    "high",
    "Add data-conversion marker or event hook to LeadForm.",
  ),
);
