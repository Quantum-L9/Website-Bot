...
function landscape(niche: string): CompetitiveLandscapeV1 {
  return {
    ...
    query_portfolio: [
      { query_id: 'q1', query: 'scrap metal near me', intent: 'local', weight: 1 },
    ],
    observations: [
      {
        observation_id: 'o1',
    ...
    selected_donors: [
      { domain: 'example.com', aggregate_visibility: 1, observation_ids: ['o1'] },
    ],
    exclusions: [],
...
function seal(
  ...
  over: Partial<{ produced_at: string; build_id: string; version: string; input_refs: ArtifactRef[] }> = {},
) {
...
test('sealed artifact is content-addressed and passes integrity verification', () => {
  ...
  assert.equal(
    ...
    `competitive_landscape:${artifact.integrity.payload_digest}`,
  );
...
test('identity ignores producer timestamp, build id, and producer version', () => {
  const a = seal(landscape('scrap-metal'), { produced_at: '2026-01-01T00:00:00.000Z', build_id: 'b-a', version: '2.1.0' });
  const b = seal(landscape('scrap-metal'), { produced_at: '2030-12-31T23:59:59.000Z', build_id: 'b-b', version: '9.9.9' });
...
test('unsupported protocol version fails schema verification', () => {
  const artifact = seal(landscape('scrap-metal'));
  const tampered = { ...artifact, protocol_version: '9.9' as typeof WEBSITE_INTELLIGENCE_PROTOCOL_VERSION };
  assert.throws(
    () => assertIntelligenceArtifactIntegrity(tampered),
