...
export class GraphitiMemoryClient {
    ...
    async health() { return this.callTool('memory.health', {}); }
    async hydrate(input) {
    ...
    async writeDecision(input) { return this.ingest('decision', input); }
    async writeOutcome(input) { return this.ingest('observation', input); }
    async writeSemanticFact(input) { return this.ingest('semantic', input); }
    ...
    async callTool(name, args) {
        ...
        try {
            ...
            if (!this.sessionId || !(error instanceof MemoryRpcError) || ![400, 404].includes(error.httpStatus ?? 0))
                throw error;
...
function escapeBoundaryText(value) { return value.replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('>', '&gt;'); }
