...
export class E2EEngine {
  ...
  private async executeTest(test: E2ETestResult): Promise<E2ETestResult> {
    this.logger.debug(
      { testId: test.test_id, command: test.command_or_invocation },
      "Executing E2E test",
    );
    ...
    try {
      ...
      const executedTest: E2ETestResult = {
        ...
        contributing_causes: this.identifyContributingCauses(
          result.exitCode,
          result.stdout,
          result.stderr,
        ),
  ...
  private classifyTestResult(
    ...
    if (
      combinedOutput.includes("unhandled promise rejection") ||
    ...
    // Dependency failures
    if (
      combinedOutput.includes("module not found") ||
    ...
    if (
      combinedOutput.includes("configuration") ||
  ...
  evaluateResults(e2eResults: E2ETestResult[]): {
    ...
    const unknownTests = e2eResults.filter((test) => test.status === "Unknown").length;
