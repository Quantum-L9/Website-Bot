...
describe("E2EEngine", () => {
  ...
  test("properly classifies test failures vs assertion failures", async () => {
    ...
    try {
      ...
      strictEqual(
        apiTest.primary_failure_classification,
        "AssertionFailure",
        "Should classify as assertion failure",
      );
      ...
      strictEqual(
        e2eTest.primary_failure_classification,
        "ApplicationRuntimeFailure",
        "Should classify as runtime failure",
      );
    ...
    try {
      ...
      for (const test of blockedTests) {
        strictEqual(
          ...
          "Should mark as blocked by preflight gate",
        );
        strictEqual(test.primary_failure_classification, "Blocked", "Should classify as blocked");
        ok(
          test.assertion_or_error?.includes("blocked by failed preflight gate"),
          "Should explain blocking reason",
        );
        strictEqual(
          test.root_cause_group,
          "preflight_gate_failure",
          "Should group by preflight failure",
        );
  ...
  test("generates comprehensive summary with correct metrics", async () => {
    ...
    try {
      ...
      strictEqual(
        summary.executed_unique_tests,
        2,
        "Should count only actually executed tests (not blocked)",
      );
