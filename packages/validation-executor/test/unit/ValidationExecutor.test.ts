...
describe("ValidationExecutor", () => {
  ...
  test("blocks E2E execution when preflight gate fails", async () => {
    ...
    try {
      ...
      ok(
        report.e2e_summary.blocked_by_preflight_gate > 0,
        "E2E tests should be blocked by preflight gate",
      );
  ...
  test("generates proper evidence references throughout execution", async () => {
    ...
    try {
      ...
      assertions.evidenceReferences(
        report.validation_gates.execution_context_resolved.evidence_references,
      );
