...
import { createMockExecutionContext, createTestEnvironment } from "../setup.js";
...
describe("AuditReporter", () => {
  test("generates complete validation execution report", async () => {
    ...
    try {
      ...
      strictEqual(
        report.execution_context.target_environment,
        "test",
        "Should preserve context data",
      );
  ...
  test("writes report to file correctly", async () => {
    ...
    try {
      ...
      strictEqual(
        parsedReport.run_metadata.run_id,
        "file-test-123",
        "File content should match report data",
      );
    } finally {
      await env.cleanup();
    }
  ...
  test("generates authority sources correctly", async () => {
    ...
    try {
      ...
      strictEqual(
        packageJsonSource.revision_or_version,
        "commit-abc123",
        "Should use source revision",
      );
      strictEqual(
        packageJsonSource.applicable_scope,
        "execution_configuration",
        "Should set correct scope",
      );
  ...
  test("detects dynamic inventory items correctly", async () => {
    ...
    try {
      ...
      ok(
        dynamicItems.some((item) => item.includes("environment-specific-config:staging")),
        "Should detect staging environment config",
      );

      // Should detect service dependencies
      ok(
        dynamicItems.some((item) => item.includes("service-dependency:redis")),
        "Should detect Redis service dependency",
      );
      ok(
        dynamicItems.some((item) => item.includes("service-dependency:postgresql")),
        "Should detect PostgreSQL service dependency",
      );

      // Should detect credential dependencies
      ok(
        dynamicItems.some((item) => item.includes("credential-dependent:DATABASE_PASSWORD")),
        "Should detect database password credential",
      );
      ok(
        dynamicItems.some((item) => item.includes("credential-dependent:API_KEY")),
