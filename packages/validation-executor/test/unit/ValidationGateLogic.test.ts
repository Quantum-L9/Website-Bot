  ...
  describe("PreflightEngine Gate Evaluation", () => {
    ...
    test("gate evaluates all failure statuses as blocking failures", async () => {
      ...
      for (const status of failureStatuses) {
        ...
        strictEqual(
          gate.e2e_authorized,
          false,
          `E2E should not be authorized for status: ${status}`,
        );
        strictEqual(
          gate.blocking_failures.length,
          1,
          `Should have blocking failure for status: ${status}`,
        );
        ok(
          gate.blocking_failures.includes(`test_${status.toLowerCase()}`),
    ...
    test("gate ignores non-blocking checks in authorization decision", async () => {
      ...
      try {
        ...
        strictEqual(
          gate.e2e_authorized,
          true,
          "E2E should be authorized despite non-blocking failures",
        );
        strictEqual(gate.blocking_failures.length, 0, "Should have no blocking failures");
        strictEqual(gate.blocking_unknowns.length, 0, "Should have no blocking unknowns");
      } finally {
    ...
    test("gate provides comprehensive evidence references", async () => {
      ...
      try {
        ...
        ok(
          gate.decision_evidence.some((e) => e.includes("preflight_gate_evaluation")),
          "Should include gate evaluation evidence",
        );
        ok(
          gate.decision_evidence.some((e) => e.includes("preflight_check1_result")),
          "Should include check1 evidence",
        );
        ok(
          gate.decision_evidence.some((e) => e.includes("preflight_check2_result")),
  ...
  describe("ValidationExecutor Final Verdict Logic", () => {
    test("determines PASS verdict when all gates pass and no failures", async () => {
      ...
      try {
        ...
        strictEqual(
          report.final_verdict.required_failure_count,
          0,
          "Should have zero failure count",
        );
    ...
    test("determines FAIL verdict when tests fail despite passing gates", async () => {
      ...
      try {
        ...
        ok(
          report.final_verdict.verdict_reason.includes("failure"),
    ...
    test("determines INCOMPLETE verdict when context resolution fails", async () => {
      ...
      try {
        ...
        strictEqual(
          ...
          "INCOMPLETE",
          "Should determine INCOMPLETE verdict",
        );
        strictEqual(
          report.validation_gates.execution_context_resolved.status,
          "Failed",
          "Context resolution should fail",
        );
        ok(
          report.final_verdict.verdict_reason.includes("incomplete"),
    ...
    test("counts failures correctly across preflight and E2E results", async () => {
      ...
      try {
        ...
        strictEqual(
          report.final_verdict.required_failure_count,
          1,
          "Should count preflight failure",
        );
        ...
        strictEqual(
          report.validation_gates.preflight_passed.status,
          "Failed",
          "Preflight gate should fail",
        );
        strictEqual(
          report.validation_gates.e2e_tests_passed.status,
          "Failed",
          "E2E gate should fail",
        );
      } finally {
        await env.cleanup();
      }
    ...
    test("provides accurate status summaries in final verdict", async () => {
      ...
      try {
        ...
        strictEqual(
          report.final_verdict.preflight_status,
          "Passed",
          "Should report preflight status",
        );
        strictEqual(report.final_verdict.e2e_status, "Passed", "Should report e2e status");
        strictEqual(
          report.final_verdict.coverage_status,
          "Passed",
          "Should report coverage status",
        );
        strictEqual(
          report.final_verdict.evidence_status,
          "Passed",
          "Should report evidence status",
        );
        ok(
          Array.isArray(report.final_verdict.blocking_defect_ids),
          "Should have blocking defect IDs array",
        );
        ok(
          typeof report.final_verdict.verdict_reason === "string",
          "Should have verdict reason string",
        );
      } finally {
        await env.cleanup();
      }
    ...
    test("E2E execution proceeds when preflight gate passes", async () => {
      ...
      try {
        ...
        strictEqual(
          report.validation_gates.preflight_passed.status,
          "Passed",
          "Preflight gate should pass",
        );
        strictEqual(
          report.validation_gates.e2e_tests_passed.status,
          "Passed",
          "E2E gate should pass",
        );
      } finally {
        await env.cleanup();
      }
    ...
    test("E2E execution blocked when preflight gate fails", async () => {
      ...
      try {
        ...
        strictEqual(
          report.validation_gates.e2e_tests_passed.status,
          "Failed",
          "E2E gate should fail due to blocking",
        );
        ...
        strictEqual(
          ...
          "E2E should be marked as blocked",
        );
      } finally {
        await env.cleanup();
      }
