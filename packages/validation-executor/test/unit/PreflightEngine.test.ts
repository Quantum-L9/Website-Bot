...
import { createTestEnvironment, createMockPreflightChecks, MockRepositoryAdapter, assertions } from '../setup.js';
...
describe('PreflightEngine', () => {
  ...
  test('evaluates gate as Passed when non-blocking checks fail', async () => {
    ...
    try {
      ...
      assertions.gateStatus(gate, 'Passed');
      strictEqual(gate.e2e_authorized, true, 'E2E should be authorized when only non-blocking checks fail');
      strictEqual(gate.blocking_failures.length, 0, 'Should have no blocking failures');
  ...
  test('evaluates gate as Failed when blocking check has Unknown status', async () => {
    ...
    try {
      ...
      assertions.gateStatus(gate, 'Failed');
      strictEqual(gate.e2e_authorized, false, 'E2E should not be authorized when blocking check is unknown');
      strictEqual(gate.blocking_failures.length, 0, 'Should have no blocking failures');
  ...
  test('executes preflight checks and classifies results correctly', async () => {
    ...
    try {
      const results = await engine.executeAll(checkDefinitions.map(def => ({
        ...def,
        status: 'NotExecuted',
        exit_code: null,
        termination_signal: null,
      ...
      strictEqual(lintResult.primary_failure_classification, 'PreflightFailure', 'Should classify as preflight failure');
  ...
  test('handles timeout scenarios correctly', async () => {
    ...
    try {
      ...
      strictEqual(result.primary_failure_classification, 'Timeout', 'Should classify failure as timeout');
