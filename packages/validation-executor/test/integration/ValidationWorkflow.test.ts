...
import { createTestEnvironment, createMockConfig, MockRepositoryAdapter } from '../setup.js';
...
describe('Validation Workflow Integration', () => {
  ...
  test('preflight failure blocks E2E execution', async () => {
    ...
    try {
      ...
      strictEqual(report.e2e_results[0].status, 'BlockedByPreflightGate', 'E2E test should be blocked');
  ...
  test('mixed preflight results with non-blocking failures', async () => {
    ...
    try {
      ...
      strictEqual(report.preflight_summary.gate_status, 'Passed', 'Gate should pass (only blocking matters)');
  ...
  test('handles timeout scenarios in validation workflow', async () => {
    ...
    try {
      ...
      strictEqual(preflightResult.primary_failure_classification, 'Timeout', 'Should classify failure as timeout');

      // Validate preflight gate failure
      strictEqual(report.preflight_summary.gate_status, 'Failed', 'Gate should fail due to timeout');
  ...
  test('generates comprehensive evidence throughout workflow', async () => {
    ...
    try {
      ...
      ok(evidenceTypes.has('execution_trace') || evidenceTypes.has('command_execution'), 
      ...
      ok(report.validation_gates.execution_context_resolved.evidence_references.length > 0,
        'Context gate should have evidence references');
      ok(report.validation_gates.preflight_passed.evidence_references.length > 0,
        'Preflight gate should have evidence references');
      ok(report.validation_gates.e2e_tests_passed.evidence_references.length > 0,
