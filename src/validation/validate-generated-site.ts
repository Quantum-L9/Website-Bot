...
} from "../pipeline/evidence/EvidenceCodec.js";
import {
  digestDirectory,
...
export function safePathSegment(value: string, label: string): string {
  ...
  if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/.test(trimmed) || trimmed === "." || trimmed === "..") {
    throw new BuildError(
      "VALIDATION_FAILED",
      `${label} must be a safe path segment: ${JSON.stringify(value)}`,
    );
  }
  return trimmed;
}
...
export function normalizeComponentName(component: string): string {
  ...
  if (!normalized)
    throw new BuildError(
      "VALIDATION_FAILED",
      `Invalid component name: ${JSON.stringify(component)}`,
    );
  return normalized;
}
...
export function normalizeSiteUrl(value: string): string {
  ...
  try {
    ...
    throw new BuildError("VALIDATION_FAILED", `Invalid site URL: ${value}`);
  }
  if (!["http:", "https:"].includes(parsed.protocol)) {
    throw new BuildError("VALIDATION_FAILED", `Unsupported site URL protocol: ${parsed.protocol}`);
  }
  if (!parsed.hostname)
    throw new BuildError("VALIDATION_FAILED", `Site URL is missing a hostname: ${value}`);
  parsed.hash = "";
  parsed.search = "";
...
export function safeChild(root: string, relativePath: string): string {
  ...
  if (candidate !== rootPath && !candidate.startsWith(`${rootPath}${sep}`)) {
    throw new BuildError(
      ...
      `Generated path escaped output directory: ${relativePath}`,
    );
  }
  return candidate;
}
...
export function validateRouteContracts(routes: RouteContract[]): string[] {
  if (routes.length === 0)
    throw new BuildError("MISSING_INPUT", "domainSpec.routes must contain at least one route");
  const seen = new Set<string>();
  return routes.map((route, index) => {
    const slug = normalizeRouteSlug(route.slug);
    if (seen.has(slug))
      throw new BuildError("VALIDATION_FAILED", `Duplicate normalized route slug: ${slug}`);
    seen.add(slug);
    if (!route.title.trim())
      throw new BuildError("VALIDATION_FAILED", `routes[${index}].title must be non-empty`);
    if (!Array.isArray(route.components))
      throw new BuildError("VALIDATION_FAILED", `routes[${index}].components must be an array`);
...
export function validateGeneratedSite(root: string, routes: RouteContract[]): void {
  ...
  for (const relativePath of REQUIRED_FILES) {
    if (!existsSync(safeChild(absoluteRoot, relativePath))) {
      throw new BuildError(
        "SITE_ASSEMBLY_FAILED",
        `Generated site is missing required file: ${relativePath}`,
      );
    }
  }
  ...
  for (const slug of normalizedRoutes) {
    ...
    if (!existsSync(safeChild(absoluteRoot, pagePath))) {
      throw new BuildError(
        "SITE_ASSEMBLY_FAILED",
        `Generated site is missing route page: ${pagePath}`,
      );
    }
  }
  ...
  for (const file of digest.files) {
    ...
    if (marker)
      throw new BuildError(
        "SITE_ASSEMBLY_FAILED",
        `Unresolved template marker in ${file.path}: ${String(marker)}`,
      );
  }
}
...
export function writeAssemblyManifest(root: string, manifest: AssemblyManifest): string {
  ...
  writeFileSync(
    path,
    `${JSON.stringify(encodeEvidenceArtifact("assembly", manifest), null, 2)}\n`,
    "utf-8",
  );
  return path;
}
...
export function readAssemblyManifest(root: string): AssemblyManifest {
  ...
  try {
    ...
    throw new BuildError(
      ...
      `Cannot read assembly manifest: ${String(error)}`,
    );
  }
  if (
    !parsed ||
    typeof parsed !== "object" ||
    (parsed as { schema?: unknown }).schema !== "website-bot.assembly-manifest/v2"
  ) {
    throw new BuildError("ASSEMBLY_PROOF_FAILED", "Assembly manifest schema is invalid");
  }
  return decodeEvidenceArtifact<AssemblyManifest>("assembly", parsed);
}
