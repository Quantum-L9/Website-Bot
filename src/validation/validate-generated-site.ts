...
export function writeAssemblyManifest(root: string, manifest: AssemblyManifest): string {
  ...
  writeFileSync(path, `${JSON.stringify(encodeEvidenceArtifact('assembly', manifest), null, 2)}\n`, 'utf-8');
  return path;
}
...
export function readAssemblyManifest(root: string): AssemblyManifest {
  ...
  if (!parsed || typeof parsed !== 'object' || (parsed as { schema?: unknown }).schema !== 'website-bot.assembly-manifest/v2') {
...
export function refreshAssemblyManifest(
  ...
  const manifest = buildAssemblyManifest(root, buildId, clientId, generatorVersion, routes, templateVersion, templateDigest);
