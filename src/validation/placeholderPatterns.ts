...
export const PLACEHOLDER_PATTERNS: readonly PlaceholderPattern[] = [
  {
    ...
    description:
      'Bracketed fill-in placeholder left in copy (e.g. "[phone number]", "[business name]")',
    severity: "error",
    regex:
      /\[(?:your |insert |add |enter )?[a-z][a-z0-9 _-]{1,40}(?:number|name|address|email|phone|city|state|url|link|date|here)\]/i,
  },
  {
    ...
    severity: "error",
    regex:
      /\{\{[^}]{1,60}\}\}|\$\{[^}]{1,60}\}|<(?:PHONE|EMAIL|ADDRESS|NAME|CITY|STATE|URL)[^>]{0,20}>/,
  },
  {
    id: "lorem-ipsum",
  ...
  },
  {
    id: "example-domain",
    description:
      "RFC 2606 example/test domain in shipped content (example.com, example.org, *.invalid, *.test)",
    severity: "error",
    regex:
      /\bhttps?:\/\/(?:[a-z0-9-]+\.)*(?:example\.(?:com|org|net)|[a-z0-9-]+\.(?:invalid|test))\b/i,
  },
  {
    id: "reserved-phone",
  ...
  },
  {
    ...
    description:
      'Form endpoint that looks like a test/placeholder ID (contains "test", "demo", "sample", "placeholder")',
    severity: "error",
    regex:
      /https?:\/\/(?:www\.)?formspree\.io\/f\/[a-z0-9-]*(?:test|demo|sample|placeholder)[a-z0-9-]*/i,
  },
  {
