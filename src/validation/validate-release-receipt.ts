...
import {
  type ReleaseReceipt,
  validateReleaseReceipt as validateContract,
} from "../pipeline/evidence/ReleaseReceipt.js";
...
function assertNoSecrets(value: unknown, path = "$"): void {
  if (Array.isArray(value)) {
    value.forEach((child, index) => {
      assertNoSecrets(child, `${path}[${index}]`);
    });
    return;
  }
  ...
  if (typeof value === "string" && SECRET_VALUE.test(value))
    throw new Error(`secret-bearing value is not allowed at ${path}`);
}
...
export function validateReleaseReceipt(
  value: unknown,
  _schemaPath?: string,
): asserts value is ReleaseReceipt {
