...
export class VercelProvisioningError extends Error {
  constructor(
    message: string,
    public readonly result?: VercelProvisioningResult,
...
export class VercelProvisioner {
  ...
  async provision(
    ...
    const query = request.vercel.teamId
      ? `?teamId=${encodeURIComponent(request.vercel.teamId)}`
      : "";
    ...
    if (!project) {
      ...
      project = (
        await requestJson<VercelProject>(
          this.fetchImpl,
          "vercel",
          `${API}/v11/projects${query}`,
          {
            method: "POST",
            headers,
            body: JSON.stringify({
  ...
  async remove(result: VercelProvisioningResult, token: string): Promise<void> {
    ...
    const response = await this.fetchImpl(
      `${API}/v9/projects/${encodeURIComponent(result.projectId)}${query}`,
      {
        method: "DELETE",
    ...
    if (response.status !== 204 && response.status !== 404) {
      throw new ProvisioningHttpError(
        "vercel",
        response.status,
        `Failed to rollback Vercel project ${result.projectId}`,
        await response.text(),
      );
    }
  }
    ...
    try {
      return (
        await requestJson<VercelProject>(
          this.fetchImpl,
          "vercel",
          `${API}/v9/projects/${encodeURIComponent(name)}${query}`,
          { headers },
          [200],
        )
      ).body;
  ...
  private assertProjectIdentity(
    ...
    if (!project.id || !project.name)
      throw new Error("Vercel project response is missing id or name");
    if (project.name !== request.vercel.project)
      throw new Error(
        `Vercel project collision: expected ${request.vercel.project}, observed ${project.name}`,
      );
    ...
    if (project.link.repo) {
      observedRepo = project.link.repo.includes("/")
        ? project.link.repo
        : `${project.link.org ?? request.github.owner}/${project.link.repo}`;
    }
    if (!observedRepo) {
    ...
    if (observedRepo.toLowerCase() !== repository.fullName.toLowerCase()) {
      throw new Error(
        `Vercel project ${project.name} is linked to ${observedRepo}, not ${repository.fullName}`,
      );
    }
    if (
      project.link?.repoId !== undefined &&
      String(project.link.repoId) !== repository.repositoryId
    ) {
      throw new Error(`Vercel project ${project.name} repository id mismatch`);
    }
    if (
      project.link?.productionBranch &&
      project.link.productionBranch !== repository.sourceBranch
    ) {
      throw new Error(
        `Vercel production branch is ${project.link.productionBranch}, expected ${repository.sourceBranch}`,
      );
    }
  }
  ...
  private async upsertEnvironment(
    ...
    if ((response.body.failed ?? []).length > 0) {
      const details = response.body.failed
        ?.map((item) => `${item.error?.key ?? "UNKNOWN"}: ${item.error?.message ?? "failed"}`)
        .join("; ");
  ...
  private assertDeployHookReference(project: VercelProject, request: ProvisioningRequest): void {
    ...
    const matching = project.link?.deployHooks?.some(
      (hook) => hook.url === expectedUrl && (!hook.ref || hook.ref === request.github.sourceBranch),
    );
    if (!matching) {
      throw new Error(
        `The Vercel deploy hook referenced by ${ref} is not registered for ${project.name}@${request.github.sourceBranch}`,
      );
    }
  }
}
