...
function envTargets(value: unknown): VercelEnvironmentTarget[] {
  ...
  const targets = value.filter(
    (item): item is VercelEnvironmentTarget =>
      typeof item === "string" && allowed.has(item as VercelEnvironmentTarget),
  );
  if (targets.length !== value.length || new Set(targets).size !== targets.length)
    throw new Error("provision.vercel.environment.targets contains invalid or duplicate values");
  return targets;
}
...
export function buildProvisioningRequest(
  ...
  if (!provision)
    throw new Error("DomainSpec.provision is required when automatic provisioning is requested");
  ...
  const publishCredentialRef =
    provision.github.publish_credential_ref?.trim() || "env://GITHUB_SITE_TOKEN";
  assertEnvRef(publishCredentialRef, "provision.github.publish_credential_ref");
  const githubCredentialRef =
    provision.maintenance?.github_credential_ref?.trim() || "env://SEO_BOT_SITE_GITHUB_TOKEN";
  ...
  if (vercelDeployHookRef)
    assertEnvRef(vercelDeployHookRef, "provision.maintenance.vercel_deploy_hook_ref");

  const environment = (provision.vercel.environment ?? []).map((item, index) => {
    if (!ENV_KEY.test(item.key))
      throw new Error(`provision.vercel.environment[${index}].key is invalid`);
  ...
  if (new Set(environment.map((item) => item.key)).size !== environment.length)
    throw new Error("provision.vercel.environment contains duplicate keys");
  ...
  return {
    ...
    rollbackCreatedResources:
      options.rollbackCreatedResources ?? provision.rollback_created_resources ?? true,
    github: {
      owner,
      repository,
      visibility: visibility(provision.github.visibility),
      description:
        provision.github.description?.trim() ||
        `Generated Astro website for ${domainSpec.business_name}`,
      sourceBranch,
