...
export class GitHubProvisioningError extends Error {
  constructor(
    message: string,
    public readonly result?: GitHubProvisioningResult,
...
export class GitHubProvisioner {
  ...
  async provision(request: ProvisioningRequest, token: string): Promise<GitHubProvisioningResult> {
    ...
    if (!repository) {
      ...
      const owner = await requestJson<GitHubIdentity>(
        this.fetchImpl,
        "github",
        `${API}/users/${encodeURIComponent(request.github.owner)}`,
        { headers },
        [200],
      );
      const body = {
      ...
      const url =
        owner.body.type === "Organization"
          ? `${API}/orgs/${encodeURIComponent(request.github.owner)}/repos`
          : `${API}/user/repos`;
      if (owner.body.type === "User") {
        const authenticated = await requestJson<GitHubIdentity>(
          this.fetchImpl,
          "github",
          `${API}/user`,
          { headers },
        ...
        if (authenticated.body.login.toLowerCase() !== request.github.owner.toLowerCase()) {
          throw new Error(
            `Cannot create a repository under user ${request.github.owner} while authenticated as ${authenticated.body.login}`,
          );
        }
      }
      repository = (
        await requestJson<GitHubRepository>(
          this.fetchImpl,
          "github",
          url,
          {
    ...
    try {
      ...
      if (repository.full_name.toLowerCase() !== fullName.toLowerCase())
        throw new Error(
          `GitHub repository identity mismatch: expected ${fullName}, observed ${repository.full_name}`,
        );
  ...
  async verifyAccess(fullName: string, token: string, label: string): Promise<void> {
    await requestJson<GitHubRepository>(
      ...
      throw new Error(
        `${label} cannot access ${fullName}: ${error instanceof Error ? error.message : String(error)}`,
      );
    });
  }
  ...
  async remove(result: GitHubProvisioningResult, token: string): Promise<void> {
    ...
    if (response.status !== 204 && response.status !== 404) {
      throw new ProvisioningHttpError(
        "github",
        response.status,
        `Failed to rollback GitHub repository ${result.fullName}`,
        await response.text(),
      );
    }
  }
  ...
  private async ensureBranch(
    ...
    try {
      await requestJson<GitRef>(
        this.fetchImpl,
        "github",
        `${API}/repos/${repository.full_name}/git/ref/heads/${encoded}`,
        { headers },
        [200],
      );
      return;
    ...
    try {
      baseSha = (
        await requestJson<GitRef>(
          this.fetchImpl,
          "github",
          `${API}/repos/${repository.full_name}/git/ref/heads/${defaultEncoded}`,
          { headers },
          [200],
        )
      ).body.object?.sha;
    ...
    if (!baseSha) await this.seedEmptyRepository(repository.full_name, branch, headers);
      ...
      await requestJson<GitObject>(
        this.fetchImpl,
        "github",
        `${API}/repos/${repository.full_name}/git/refs`,
        {
          method: "POST",
          headers,
    ...
    if (repository.default_branch !== branch) {
      await requestJson<GitHubRepository>(
        this.fetchImpl,
        "github",
        `${API}/repos/${repository.full_name}`,
        {
          method: "PATCH",
          headers,
  ...
  private async seedEmptyRepository(
    fullName: string,
    branch: string,
    headers: Record<string, string>,
  ): Promise<string> {
    const blob = (
      await requestJson<GitObject>(
        this.fetchImpl,
        "github",
        `${API}/repos/${fullName}/git/blobs`,
        {
          method: "POST",
          headers,
          body: JSON.stringify({ content: "# Provisioned client site\n", encoding: "utf-8" }),
        },
        [201],
      )
    ).body.sha;
    ...
    const commit = (
      await requestJson<GitObject>(
        this.fetchImpl,
        "github",
        `${API}/repos/${fullName}/git/commits`,
        {
          method: "POST",
          headers,
          body: JSON.stringify({
            message: "chore: initialize provisioned client site",
            tree,
            parents: [],
          }),
        },
