...
export function matchSourcePage(
  manifest: SourceSiteManifest | undefined,
  slug: string,
): IngestedPage | undefined {
  if (!manifest?.pages.length) return undefined;
  const wanted = slug.replace(/\/$/, "") || "/";
  return (
    manifest.pages.find((page) => pagePath(page.url) === wanted) ??
...
export function topNavFromSource(
  manifest: SourceSiteManifest | undefined,
...
export function faqsFromManifest(
  manifest: SourceSiteManifest | undefined,
): Array<{ question: string; answer: string }> {
  if (!manifest?.pages.length) return [];
  const faqPage =
    manifest.pages.find((page) => {
...
export function assembleSourceSection(page: IngestedPage, component: string): string {
  ...
  if (/^(cta|final_cta|contact_form)$/.test(key)) {
    const rest =
      paras[0] && !LAME_CTA.test(paras[0])
        ? paras[0]
        : "Call or text for a free inspection. We respond within 24 hours.";
    return stripMarkdownDecorators(`${OFFER_HEADLINE}\n\n${rest}`);
  }
  if (key === "gallery") {
    return stripMarkdownDecorators(
      `${page.headings.find((h) => /work|gallery|before/i.test(h)) || "Our Work"}\n\n${paras[0] || ""}`.trim(),
    );
  }
  if (key === "hero") {
