...
export class SEOBaselineStage implements Stage {
  ...
  async run(ctx: BuildContext): Promise<void> {
    ...
    const keywords =
      configured.length > 0
        ? configured
        : ctx.domainSpec.routes.map((route) => `${route.title} ${geography.primary_state}`);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        logger.warn(
          ...
          "DataForSEO request failed; SEO baseline marked failed",
        );
        return;
      }
      const data = (await response.json()) as {
        tasks?: Array<{
          result?: Array<{
            items?: Array<{ type: string; rank_absolute: number; domain?: string }>;
          }>;
        }>;
      };
      ...
      let siteHost = "";
      if (rawSiteUrl) {
        const normalizedSiteUrl = /^[A-Za-z][A-Za-z0-9+.-]*:/.test(rawSiteUrl)
          ? rawSiteUrl
          : `https://${rawSiteUrl}`;
      ...
      for (let index = 0; index < targetKeywords.length; index++) {
        ...
        const item = data.tasks?.[index]?.result?.[0]?.items?.find(
          (result) =>
            result.type === "organic" && Boolean(siteHost) && result.domain?.includes(siteHost),
        );
      ...
      logger.info(
        {
          keywords: Object.keys(ranks).length,
          found: Object.values(ranks).filter((value) => value !== null).length,
        },
        "SEO baseline completed",
      );
    } catch (error) {
      for (const keyword of targetKeywords) ranks[keyword] = null;
      ctx.baselineRanks = ranks;
