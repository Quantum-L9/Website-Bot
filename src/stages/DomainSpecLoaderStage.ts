...
export class DomainSpecLoaderStage implements Stage {
  ...
  async run(ctx: BuildContext): Promise<void> {
    ...
    let raw: string;
    try {
      raw = readFileSync(this.specPath, "utf-8");
    } catch (error) {
      throw new BuildError(
        "SPEC_LOAD_FAILED",
        `Cannot read spec at ${this.specPath}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    let parsed: unknown;
    try {
      ...
      throw new BuildError(
        "VALIDATION_FAILED",
        `YAML parse error in ${this.specPath}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    ...
    if (ctx.clientId && ctx.clientId !== spec.client_id) {
      throw new BuildError(
        ...
        `BuildContext clientId (${ctx.clientId}) differs from DomainSpec (${spec.client_id})`,
      );
    }
    ctx.domainSpec = spec;
    ...
    if (spec.deploy?.github_repo) {
      ctx.deployTarget = {
        ...
        seoBotGithubCredentialRef:
          spec.deploy.seo_bot_github_credential_ref ?? "env://SEO_BOT_SITE_GITHUB_TOKEN",
        seoBotVercelDeployHookRef: spec.deploy.seo_bot_vercel_deploy_hook_ref,
      };
    }
    logger.info(
      { clientId: spec.client_id, routes: spec.routes.length, vertical: spec.vertical },
      "Domain spec loaded",
    );
  }
}
