  ...
  async run(ctx: BuildContext): Promise<void> {
    ...
    if (ctx.qualityEvidence.visualQa !== "passed") {
      throw new BuildError(
        ...
        `Release cannot finalize because visual QA is ${ctx.qualityEvidence.visualQa}`,
      );
    }
    if (!receipt.correlation.all_required_identities_match) {
      throw new BuildError(
        "EVIDENCE_IDENTITY_MISMATCH",
        "Release cannot finalize because evidence identities do not match",
      );
    }
    ...
    logger.info(
      { receiptId: finalized.receipt_id, qa: finalized.qa },
      "Release receipt finalized and chain revalidated",
    );
  }
}
