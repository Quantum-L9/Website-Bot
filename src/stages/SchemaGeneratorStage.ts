...
export class SchemaGeneratorStage implements Stage {
  ...
  async run(ctx: BuildContext): Promise<void> {
    ...
    const reconstructing =
      Boolean(ctx.sourceSiteManifest?.pages.length) ||
  ...
  private async generateFaqs(
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      try {
        ...
        if (attempt > 1)
          logger.warn({ attempt }, "FAQ generation succeeded after corrective retry");
        return faqs;
      } catch (error) {
    ...
    throw new BuildError(
      ...
      `FAQ JSON parse or shape failed after ${maxAttempts} attempts: ${lastFailure}`,
      true,
    );
  }
}
