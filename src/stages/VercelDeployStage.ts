...
export class VercelDeployStage implements Stage {
  ...
  version = "4.0.0";
  evidence = {
    inputs: (_ctx: BuildContext) => ["publication" as const],
    outputs: (_ctx: BuildContext) => ["deployment" as const],
    resumable: true,
  ...
  async run(ctx: BuildContext): Promise<void | StageRunResult> {
    ...
    if (!target)
      throw new BuildError("VERCEL_DEPLOY_FAILED", "Per-client deploy target is missing");
    if (ctx.dryRun) {
      logger.info(
        { projectId: target.vercelProjectId, branch: target.sourceBranch },
        "[dry-run] Would deploy published client commit to Vercel",
      );
      return;
    }
    ...
    if (!storedPublication)
      throw new BuildError(
        ...
        "Persisted publication evidence is required before Vercel deployment",
      );
    ...
    if (target.vercelDeployHook) {
      ...
      if (!token || !target.vercelProjectId || !target.githubRepoId) {
        throw new BuildError(
          ...
          "VERCEL_TOKEN, deploy.vercel_project_id, and deploy.github_repo_id are required for API deployment",
        );
      }
    ...
    const finalState = await this.pollDeployment(
      deploymentId,
      token,
      teamId,
      target.vercelProjectId,
    ...
    if (
      ...
      target.vercelProjectId &&
      finalState.projectId !== target.vercelProjectId
    ) {
      throw new BuildError(
        "DEPLOYMENT_CORRELATION_FAILED",
        "Vercel deployment project does not match configured project",
        false,
        {
    ...
    const aliases = [
      ...new Set(
        [...(finalState.alias ?? []), ...(finalState.aliases ?? [])].map((value) =>
    ...
    if (!deploymentUrl)
      throw new BuildError(
        "DEPLOYMENT_CORRELATION_FAILED",
        "Vercel deployment did not report a URL",
      );
    const projectId = target.vercelProjectId ?? finalState.projectId;
    if (!projectId)
      throw new BuildError(
        "DEPLOYMENT_CORRELATION_FAILED",
        "Vercel deployment did not report a project ID",
      );
  ...
  async canResume(ctx: BuildContext): Promise<boolean> {
    ...
    try {
      const status = await this.requestJson<VercelDeploymentResponse>(
        `${VERCEL_API}/v13/deployments/${encodeURIComponent(stored.value.deploymentId)}${this.teamQuery(process.env.VERCEL_TEAM_ID)}`,
        { headers: { Authorization: `Bearer ${token}` } },
        "Vercel resume verification failed",
      );
      return (
        (status.readyState ?? status.state) === "READY" &&
        this.extractCommitSha(status) === stored.value.observedCommitSha
      );
    } catch {
      return false;
    }
  ...
  private resolveTarget(): "preview" | "production" {
    ...
    if (value !== "preview" && value !== "production")
      throw new BuildError(
        "VERCEL_DEPLOY_FAILED",
        `VERCEL_TARGET must be preview or production, received ${value}`,
      );
    if (value === "production" && process.env.WEBSITE_BOT_ALLOW_PRODUCTION !== "true") {
      throw new BuildError(
        "VERCEL_DEPLOY_FAILED",
        "Production deployment requires WEBSITE_BOT_ALLOW_PRODUCTION=true",
      );
    }
    return value;
  }
  ...
  private async triggerHookAndCorrelate(
    ...
    try {
      ...
      throw new BuildError("VERCEL_DEPLOY_FAILED", "Vercel deploy hook is not a valid URL");
    }
    if (parsed.protocol !== "https:")
      throw new BuildError("VERCEL_DEPLOY_FAILED", "Vercel deploy hook must use HTTPS");
    const response = await this.fetchImpl(parsed, { method: "POST" });
    if (!response.ok)
      throw new BuildError("VERCEL_DEPLOY_FAILED", `Vercel deploy hook failed: ${response.status}`);
    ...
    if (!jobId)
      throw new BuildError(
        "DEPLOYMENT_CORRELATION_FAILED",
        "Vercel deploy hook response did not include a job ID",
      );
    if (!token || !projectId) {
      throw new BuildError(
        "DEPLOYMENT_CORRELATION_FAILED",
        "Deploy hook acceptance is not release proof; VERCEL_TOKEN and project ID are required to correlate the resulting deployment",
        false,
        { jobId },
      );
    }
    ...
    for (let attempt = 1; attempt <= this.maxPolls; attempt++) {
      ...
      const match = (list.deployments ?? []).find(
        (candidate) => this.extractCommitSha(candidate) === commitSha,
      );
    ...
    throw new BuildError(
      "DEPLOYMENT_CORRELATION_FAILED",
      "Deploy hook was accepted but no deployment matching the published commit was observed",
      false,
      { jobId, commitSha },
    );
  }
  ...
  private async pollDeployment(
    ...
    if (!token)
      throw new BuildError(
        "DEPLOYMENT_CORRELATION_FAILED",
        "VERCEL_TOKEN is required to observe terminal deployment state",
      );
    for (let attempt = 1; attempt <= this.maxPolls; attempt++) {
      const status = await this.requestJson<VercelDeploymentResponse>(
      ...
      if (state === "ERROR" || state === "CANCELED") {
        throw new BuildError(
          "VERCEL_DEPLOY_FAILED",
          `Deployment ${deploymentId} entered ${state}`,
          false,
          { projectId },
        );
      }
    ...
    throw new BuildError(
      "VERCEL_POLL_TIMEOUT",
      `Deployment ${deploymentId} did not become READY after ${this.maxPolls} polls`,
    );
  }
  ...
  private extractCommitSha(value: VercelDeploymentResponse): string | undefined {
    ...
    return candidates.find(
      (candidate) => typeof candidate === "string" && /^[0-9a-f]{40}$/i.test(candidate as string),
    ) as string | undefined;
  }
  ...
  private deploymentId(value: VercelDeploymentResponse): string {
    ...
    if (!id || typeof id !== "string")
      throw new BuildError(
        "DEPLOYMENT_CORRELATION_FAILED",
        "Vercel response did not include a deployment ID",
      );
    return id;
  }
  ...
  private normalizeUrl(value: string): string {
    ...
    try {
      ...
      throw new BuildError("VERCEL_DEPLOY_FAILED", `Vercel returned a malformed deployment URL`);
    }
    if (parsed.protocol !== "https:")
      throw new BuildError("VERCEL_DEPLOY_FAILED", "Vercel deployment URL must use HTTPS");
