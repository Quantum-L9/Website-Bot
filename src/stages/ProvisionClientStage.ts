...
export class ProvisionClientStage implements Stage {
  ...
  async run(ctx: BuildContext): Promise<void> {
    try {
      ...
      if (!receipt.github || !receipt.vercel)
        throw new Error("Provisioning receipt is missing GitHub or Vercel target");
      ctx.domainSpec.deploy = {
        ...
        ...(request.maintenance.vercelDeployHookRef
          ? { seo_bot_vercel_deploy_hook_ref: request.maintenance.vercelDeployHookRef }
          : {}),
      };
      ctx.deployTarget = {
        ...
        ...(request.maintenance.vercelDeployHookRef
          ? { seoBotVercelDeployHookRef: request.maintenance.vercelDeployHookRef }
          : {}),
      };
      logger.info(
        {
          clientId: ctx.clientId,
          repository: receipt.github.fullName,
          project: receipt.vercel.projectId,
          status: receipt.status,
        },
        "Client provisioning converged",
      );
    } catch (error) {
