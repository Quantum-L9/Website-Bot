...
export class SiteBuildStage implements Stage {
  ...
  async run(ctx: BuildContext): Promise<void> {
    ...
    ctx.assemblyManifest = refreshAssemblyManifest(ctx.outputDir, ctx.buildId, ctx.clientId, generatorVersion, ctx.domainSpec.routes, ctx.assemblyManifest.templateVersion, ctx.assemblyManifest.templateDigest);
