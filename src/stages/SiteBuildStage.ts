...
export class SiteBuildStage implements Stage {
  ...
  version = "3.0.0";
  evidence = {
    inputs: (_ctx: BuildContext) => ["assembly" as const],
    outputs: (_ctx: BuildContext) => ["build" as const],
    resumable: true,
  ...
  async run(ctx: BuildContext): Promise<void> {
    if (ctx.dryRun) {
      logger.info(
        ...
        "[dry-run] Would install dependencies, run Astro check, and build",
      );
      return;
    }
    ...
    if (!storedAssembly)
      throw new BuildError(
        ...
        "SiteBuildStage requires persisted assembly evidence",
      );
    ctx.assemblyManifest = storedAssembly.value;
    const packagePath = join(ctx.outputDir, "package.json");
    if (!existsSync(packagePath))
      throw new BuildError("BUILD_FAILED", `Generated package.json not found in ${ctx.outputDir}`);
    try {
      JSON.parse(readFileSync(packagePath, "utf-8"));
    } catch (error) {
      throw new BuildError("BUILD_FAILED", `Generated package.json is invalid: ${String(error)}`);
    }
    ...
    const installResult = await this.runner.run(installCommand[0], installCommand.slice(1), {
      cwd: ctx.outputDir,
      timeoutMs: this.timeoutMs,
      env: environment,
    });
    const generatorVersion = ctx.assemblyManifest?.generatorVersion ?? "Unknown";
    ctx.assemblyManifest = refreshAssemblyManifest(
      ctx.outputDir,
      ctx.buildId,
      ctx.clientId,
      generatorVersion,
      ctx.domainSpec.routes,
      ctx.assemblyManifest.templateVersion,
      ctx.assemblyManifest.templateDigest,
    );
    ...
    const checkResult = await this.runner.run(checkCommand[0], checkCommand.slice(1), {
      cwd: ctx.outputDir,
      timeoutMs: this.timeoutMs,
      env: environment,
    });
    const buildResult = await this.runner.run(buildCommand[0], buildCommand.slice(1), {
      cwd: ctx.outputDir,
      timeoutMs: this.timeoutMs,
      env: environment,
    });
    ...
    if (!existsSync(distDir) || !statSync(distDir).isDirectory())
      throw new BuildError("BUILD_FAILED", `Astro build completed without dist/: ${distDir}`);
    ...
    for (const route of ctx.domainSpec.routes) {
      ...
      if (!existsSync(absolutePath) || !statSync(absolutePath).isFile()) {
        throw new BuildError(
          "BUILD_FAILED",
          `Astro build is missing route output: ${relativePath}`,
        );
      }
      builtRoutes.push(route.slug);
    }
    if (!existsSync(join(distDir, "index.html")))
      throw new BuildError("BUILD_FAILED", "Astro dist/ has no HTML entrypoint");
    const sitemapPath = join(distDir, "sitemap-index.xml");
    if (!existsSync(sitemapPath))
      throw new BuildError("BUILD_FAILED", "Astro dist/ is missing sitemap-index.xml");
    ...
    if (source.digest !== ctx.assemblyManifest.sourceDigest) {
      throw new BuildError(
        ...
        "Source digest does not match refreshed assembly manifest",
      );
    }
    const dist = digestDirectory(distDir);
    const versionResult = await this.runner.run("npm", ["--version"], {
      cwd: ctx.outputDir,
      timeoutMs: 30_000,
      env: environment,
    });
    ...
    logger.info(
      { distDir, sourceDigest: source.digest, distDigest: dist.digest, routes: builtRoutes.length },
      "Astro local proof passed",
    );
  }
}
