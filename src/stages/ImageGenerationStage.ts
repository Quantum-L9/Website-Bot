...
import { buildImageAssetManifest, type ResolvedImageAsset } from '../pipeline/evidence/ImageAssetManifest.js';
...
function canonicalJson(value: unknown): string {
  ...
  if (value && typeof value === 'object') {
    return `{${Object.keys(value as Record<string, unknown>).sort((a, b) => (a < b ? -1 : a > b ? 1 : 0)).map(key => `${JSON.stringify(key)}:${canonicalJson((value as Record<string, unknown>)[key])}`).join(',')}}`;
  }
...
export class ImageGenerationStage implements Stage {
  ...
  async run(ctx: BuildContext): Promise<void> {
    ...
    const generatedSlots = plan?.assets.filter(asset => asset.resolution.source === 'generated') ?? [];
    ...
    const generator = this.generatorOverride ?? createImageGenerator({ model: assets.generation?.model });
    if (!generator) {
      throw new BuildError('MISSING_INPUT', 'Image generation is required by the plan but no image provider is configured (set GEMINI_API_KEY)');
    }
    ...
    for (const planned of generatedSlots) {
      ...
      const resolved = await this.resolveAsset(ctx, generator, budget, cacheRoot, slot, brief, compiled.prompt, model, fingerprint);
  ...
  private async resolveAsset(
    ...
    if (cached) {
      ...
      let result;
      try {
        result = await generator.generate({ prompt, aspectRatio: brief.aspectRatio, imageSize: brief.imageSize });
      } catch (error) {
        throw new BuildError('CONTENT_GENERATION_FAILED', `Image generation failed for slot ${slot.id}: ${String(error)}`);
      }
      cost = result.estimatedCostUsd ?? 0;
      try {
        budget.charge(cost);
      } catch (error) {
        if (error instanceof ImageBudgetExceededError) throw new BuildError('VALIDATION_FAILED', error.message);
        throw error;
      }
      bytes = result.bytes;
      ...
      const metadata: CacheMetadata = { mimeType, model: usedModel, prompt, estimatedCostUsd: cost };
  ...
  private readCache(cacheRoot: string, fingerprint: string, metaPath: string): (CacheMetadata & { bytes: Buffer }) | undefined {
