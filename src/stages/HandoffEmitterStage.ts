...
function assertAcknowledgement(
  ack: SeoBotRegistrationAck,
  expected: WebsiteFactoryHandoffV3,
): void {
  validateSeoBotRegistrationAck(ack);
  if (ack.client_id !== expected.client.id)
    throw new Error("SEO-Bot acknowledged a different client");
  if (ack.contract_id !== expected.contract_id)
    throw new Error("SEO-Bot acknowledged a different contract_id");
  if (ack.contract_digest !== expected.integrity.payload_digest)
    throw new Error("SEO-Bot acknowledged a different contract digest");
  if (ack.release_receipt_id !== expected.proof.receipt_id)
    throw new Error("SEO-Bot acknowledged a different release receipt");
  if (ack.verified_repository !== expected.site.repository.full_name)
    throw new Error("SEO-Bot verified a different repository");
  if (ack.verified_branch !== expected.site.repository.branch)
    throw new Error("SEO-Bot verified a different branch");
  if (ack.verified_commit_sha !== expected.site.repository.commit_sha)
    throw new Error("SEO-Bot verified a different commit");
}
...
export class HandoffEmitterStage implements Stage {
  ...
  evidence = {
    ...
    outputs: (ctx: BuildContext) =>
      ctx.autoRegisterSeoBot
  ...
  async canResume(ctx: BuildContext, _checkpoint: StageCheckpoint): Promise<boolean> {
    ...
    if (
      handoff.proof.receipt_id !== bundle.releaseReceipt.receipt_id ||
      handoff.site.repository.commit_sha !== bundle.publicationEvidence?.commitSha
    )
      return false;
    ...
    try {
      assertAcknowledgement(acknowledgement, handoff);
      return true;
    } catch {
      return false;
    }
  ...
  async run(ctx: BuildContext): Promise<void> {
    if (ctx.dryRun) {
      logger.info(
        ...
        "[dry-run] Would emit a canonical v3 handoff from persisted release evidence",
      );
      return;
    }
  ...
  private async registerSeoBot(ctx: BuildContext, contract: WebsiteFactoryHandoffV3): Promise<void> {
    ...
    if (!seoBotUrl || !seoBotKey) {
      throw new BuildError(
        "HANDOFF_EMIT_FAILED",
        "SEO_BOT_URL and SEO_BOT_API_KEY are required when auto-registration is enabled",
      );
    }
    ...
    try {
      ...
      throw new BuildError(
        "HANDOFF_EMIT_FAILED",
        `SEO-Bot registration request failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    ...
    if (!response.ok)
      throw new BuildError(
        "HANDOFF_EMIT_FAILED",
        `SEO-Bot rejected canonical handoff (${response.status}): ${raw.slice(0, 2_000)}`,
      );
    ...
    try {
      ...
      throw new BuildError(
        ...
        `SEO-Bot acknowledgement validation failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    ...
    writeFileSync(
      this.ackPath,
      `${JSON.stringify({ authoritative_evidence: authoritativeEvidence, ...acknowledgement }, null, 2)}\n`,
      "utf-8",
    );
    logger.info(
      {
