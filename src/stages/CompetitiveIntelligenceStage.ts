...
import { websiteImproveTask, WEBSITE_IMPROVE_LLM_POLICY } from '../intelligence/improve-llm-policy.js';
...
const ALLOWED_DISPOSITIONS = [
  'PORT', 'PORT_WITH_HARDENING', 'CONFIGURE', 'MERGE_WITH_EXISTING',
  'KEEP_LOCAL', 'MIGRATION_CONTEXT', 'REJECT', 'UNKNOWN',
] as const;
...
function parsePatterns(value: unknown, source: string): HarvestedPattern[] {
  if (!Array.isArray(value)) throw new BuildError('INTELLIGENCE_PARSE_FAILED', `${source}: expected a JSON array of patterns`);
  return value.map((entry, index) => {
    if (!isRecord(entry)) throw new BuildError('INTELLIGENCE_PARSE_FAILED', `${source}: pattern ${index} is not an object`);
    const disposition = String(entry.disposition ?? '');
    if (!(ALLOWED_DISPOSITIONS as readonly string[]).includes(disposition)) {
      throw new BuildError('INTELLIGENCE_PARSE_FAILED', `${source}: pattern ${index} has invalid disposition ${JSON.stringify(disposition)}`);
    }
    return {
...
function conversionOf(value: unknown): WebsiteBuildBlueprintV1['conversion'] {
  const row = isRecord(value) ? value : {};
  return {
    primary_action: typeof row.primary_action === 'string' ? row.primary_action : 'Request a free inspection',
...
function validateWebsiteBuildBlueprint(
  ...
  if (payload.build_intent !== 'REDESIGN_IMPROVE') {
    throw new BuildError('BLUEPRINT_GATE_FAILED', 'blueprint build_intent must be REDESIGN_IMPROVE');
  }
  if (payload.competitive_landscape_ref.artifact_id !== refForArtifact(landscape).artifact_id) {
    throw new BuildError('BLUEPRINT_GATE_FAILED', 'blueprint references a different competitive landscape');
  }
  ...
  if (actualIds.size !== expectedIds.size || [...expectedIds].some(id => !actualIds.has(id))) {
    throw new BuildError('BLUEPRINT_GATE_FAILED', 'blueprint route set must equal the spec route set');
  }
  ...
  for (const route of payload.routes) {
    for (const section of route.sections) {
      for (const ref of section.pattern_refs) {
        if (!patternIds.has(ref)) throw new BuildError('BLUEPRINT_GATE_FAILED', `section ${section.section_id} references unknown pattern ${ref}`);
      }
    }
  }
  const adopted = portfolio.patterns.filter(pattern => !['REJECT', 'UNKNOWN'].includes(pattern.disposition));
  for (const pattern of adopted) {
    if (!pattern.acceptance_test.trim()) {
      throw new BuildError('BLUEPRINT_GATE_FAILED', `adopted pattern ${pattern.pattern_id} lacks an acceptance test`);
    }
  }
}
...
export class CompetitiveIntelligenceStage implements Stage {
  ...
  constructor(private readonly portFactory: (ctx: BuildContext) => SeoBuildIntelligencePort = defaultPort) {}

  async run(ctx: BuildContext): Promise<void> {
    if (ctx.buildIntent !== 'REDESIGN_IMPROVE') {
      logger.info({ intent: ctx.buildIntent }, 'COPY intent — competitive intelligence stage not required');
      return;
    }
    ...
    const keywords = (ctx.domainSpec.seo_contract?.target_keywords ?? []).filter(keyword => keyword.trim());
    const seedQueries = keywords.length > 0
      ? keywords.slice(0, 10)
      : ctx.domainSpec.routes.slice(0, 10).map(route => `${route.title} ${ctx.domainSpec.geography.primary_state}`);
    if (seedQueries.length === 0) throw new BuildError('INTELLIGENCE_UNAVAILABLE', 'REDESIGN_IMPROVE requires target keywords or routes to seed competitive research');

    logger.info({ donors: 10, queries: seedQueries.length }, 'Fetching competitive landscape from SEO-Bot');
    ...
    if (donors.length === 0) throw new BuildError('INTELLIGENCE_EVIDENCE_INCOMPLETE', 'competitive landscape selected no donors');
    logger.info({ donors: donors.length, landscapeId: landscape.artifact_id }, 'Competitive landscape sealed');
    ...
    const routes: WebsiteBlueprintRoute[] = expectedRoutes.map(expected => {
      ...
      const sections: WebsiteBlueprintSection[] = (Array.isArray(produced?.sections) ? produced.sections : []).map((section, index) => {
        ...
        return {
          ...
          proof_requirements: Array.isArray(row.proof_requirements) ? row.proof_requirements.map(String) : [],
        };
      });
      return { route_id: expected.route_id, path: expected.path, purpose: expected.purpose, sections };
    });
    ...
    logger.info({ artifactId: blueprint.artifact_id, patterns: portfolio.patterns.length, routes: routes.length }, 'Website build blueprint sealed and gated');
  }
}
