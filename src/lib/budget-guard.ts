...
export class AgentBudgetGuard {
  ...
  reserve(estimatedUsd: number): void {
    ...
    if (estimatedUsd > remaining) {
      this.updateMode();
      const available = this.capUsd - this.actualUsd - this.reservedUsd;
      if (estimatedUsd > available)
        throw new BudgetExceededError(
          `Reservation denied: need $${estimatedUsd.toFixed(4)}, remaining $${available.toFixed(4)}, mode=${this.mode}, job=${this.jobId}`,
        );
    }
  ...
  async close(): Promise<void> {
    ...
    try {
      ...
      logger.error(
        { jobId: this.jobId, error: error instanceof Error ? error.message : String(error) },
        "Budget persistence failed",
      );
    }
  }
  ...
  private updateMode(): void {
    ...
    else if (pressure < 1) this.mode = "require_approval";
    else this.mode = "stop";
  }
}
