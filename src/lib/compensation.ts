...
export class CompensationRegistry {
  ...
  async compensate(): Promise<Array<{ stepId: string; error?: string }>> {
    const results: Array<{ stepId: string; error?: string }> = [];
    for (const entry of [...this.entries].reverse()) {
      try {
        await entry.action();
        results.push({ stepId: entry.stepId });
        logger.info({ jobId: this.jobId, stepId: entry.stepId }, "Compensation completed");
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        results.push({ stepId: entry.stepId, error: message });
        logger.error(
          { jobId: this.jobId, stepId: entry.stepId, error: message },
          "Compensation failed",
        );
      }
    }
