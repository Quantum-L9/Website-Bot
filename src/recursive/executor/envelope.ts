...
export function evaluateMutationEnvelope(pack: PEPack, patch: ProposedPatch): EnvelopeVerdict {
  ...
  for (const file of patch.changedFiles) {
    ...
    if (envelope.allowedPaths.length > 0 && !envelope.allowedPaths.some(prefix => pathWithin(file, prefix))) {
      violations.push(`path outside allowed envelope: ${file}`);
    }
  }

  if (patch.changedFiles.length > envelope.maxChangedFiles) {
    violations.push(`changed ${patch.changedFiles.length} files, envelope allows ${envelope.maxChangedFiles}`);
  }
  ...
  if (envelope.architectureExpansionAllowed !== false) {
    violations.push('architecture expansion must be prohibited (architectureExpansionAllowed must be false)');
  }
