...
export class BoundedCodingExecutor {
  ...
  executePatch(input: {
    ...
    if (beforeFullSha !== input.baseSha) {
      return {
        applied: false,
        violations: [`clone base ${beforeFullSha} != bound base ${input.baseSha}`],
      };
    }
    input.instruction.apply(clone);
    const patchedFullSha = this.commitPatch(clone, input.pack.packId);
    const diffDigest = sha256Text(
      input.instruction.changedFiles
        .map((file) => `${file}:${sha256File(resolve(clone, file))}`)
        .join("\n"),
    );
    ...
    return {
      ...
      outcome: {
        beforeFullSha,
        patchedFullSha,
        changedFiles: [...input.instruction.changedFiles].sort((left, right) =>
  ...
  private commitPatch(workdir: string, packId: string): string {
    ...
    execTrusted("git", [
      ...
      workdir,
      "-c",
      "user.email=recursive@local",
      "-c",
      "user.name=recursive-runner",
      "commit",
      "--quiet",
      "-m",
      `bounded patch for ${packId}`,
    ]);
  ...
  return {
    ...
    validation: {
      ...
      protectedHoldout: {
        verdict: "FAIL",
        caseRefs: [],
        summary: "not yet independently verified",
      },
