...
function resolveRef(root: JsonSchema, ref: string): JsonSchema {
  ...
  return ref
    ...
    .reduce<JsonSchema>((current, key) => {
      ...
      return (current.$defs ?? {})[decoded] ?? (current as unknown as Record<string, JsonSchema>)[decoded];
    }, root);
}

export function validateAgainstSchema(schema: JsonSchema, value: unknown, path = '$'): string | null {
  const root = schema;
  const walk = (node: JsonSchema, item: unknown, itemPath: string): string | null => {
    ...
    if (node.type === 'object') {
      ...
      for (const key of node.required ?? []) if (!(key in item)) return `${itemPath}.${key} is required`;
      if (node.additionalProperties === false) {
        for (const key of Object.keys(item)) if (!(key in (node.properties ?? {}))) return `${itemPath}.${key} is not allowed`;
      }
    ...
    if (node.type === 'string') {
      ...
      if (node.format === 'date-time' && Number.isNaN(Date.parse(item))) return `${itemPath} must be date-time`;
    }
    if (node.type === 'integer' && (!Number.isInteger(item) || (item as number) < (node.minimum ?? -Infinity) || (item as number) > (node.maximum ?? Infinity))) {
      return `${itemPath} must be integer in range`;
    }
    if (node.type === 'number' && (typeof item !== 'number' || item < (node.minimum ?? -Infinity) || item > (node.maximum ?? Infinity))) {
...
export function loadRecursiveSchema(name: 'engineering-signal' | 'pe-pack' | 'code-change-outcome' | 'recursive-engineering-event' | 'recursive-engineering-wave' | 'recursive-engineering-run'): JsonSchema {
  const path = resolve('schemas/recursive', `${name}.schema.json`);
  return JSON.parse(readFileSync(path, 'utf-8')) as JsonSchema;
}

export function assertSchemaConformance(schemaName: Parameters<typeof loadRecursiveSchema>[0], artifact: unknown): void {
