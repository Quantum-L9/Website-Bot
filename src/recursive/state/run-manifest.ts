...
export function validateCampaignManifest(value: unknown): asserts value is CampaignManifest {
  ...
  if (!manifest.sourceUrl || typeof manifest.sourceUrl !== 'string') throw new Error('campaign manifest source URL is invalid');
  if (manifest.mode !== 'DEVELOPMENT_RECURSIVE') throw new Error('campaign manifest mode is invalid');
  ...
  if (!manifest.state || !isLegalWave(manifest.state.currentWave) || !TERMINAL.has(manifest.state.status as RunStatus) && manifest.state.status !== 'RUNNING') {
    throw new Error('campaign manifest state is invalid');
  }
  for (const [key, value] of Object.entries(manifest.versionBinding ?? {})) {
    if (key.endsWith('FullSha') && !SHA1.test(String(value))) throw new Error(`campaign manifest ${key} is not a full SHA`);
  }
}
