...
import { isLegalWave, type CampaignManifest, type RunStatus, type WavePhase, type WaveState } from './run-manifest.js';

const PHASE_ORDER: readonly WavePhase[] = ['E2E', 'HARVEST', 'SIGNAL_DECISION', 'PE_PACK', 'PATCH', 'VERIFY', 'MERGE', 'DEPLOY', 'DEPLOY_VERIFY'];
 ...
 */
export function applyTransition(manifest: CampaignManifest, action: TransitionAction, now?: string): TransitionResult {
  const stamp = () => {
  ...
  switch (action.kind) {
    ...
    case 'SIGNAL_DECISION_NO_PACK':
      if (state.phase !== 'SIGNAL_DECISION') return reject(`signal decision is not due in phase ${state.phase}`);
      manifest.state.status = state.reviewableBeforePatch ? 'REVIEWABLE_NO_MATERIAL_ENGINEERING_SIGNAL' : 'NO_ACTIONABLE_SIGNAL';
      stamp();
      return { applied: true, status: manifest.state.status };
    ...
    case 'PE_PACK_COMPILED':
      if (state.phase !== 'SIGNAL_DECISION') return reject(`PE pack is not due in phase ${state.phase}`);
      ensure();
    ...
    case 'PATCH_VALIDATION_FAILED':
      if (state.phase !== 'VERIFY') return reject(`patch validation is not due in phase ${state.phase}`);
      manifest.state.status = 'PATCH_VALIDATION_FAILED';
      stamp();
      return { applied: true, status: manifest.state.status };

    case 'SCOPE_INSUFFICIENT':
      if (state.phase !== 'VERIFY') return reject(`scope verdict is not due in phase ${state.phase}`);
      manifest.state.status = 'PATCH_VALIDATION_FAILED';
      stamp();
      return { applied: true, status: manifest.state.status };

    case 'VERIFICATION_PASSED':
      if (state.phase !== 'VERIFY') return reject(`verification is not due in phase ${state.phase}`);
      ensure();
      state.phase = 'MERGE';
      stamp();
      return { applied: true };
    ...
    case 'DEPLOYMENT_VERIFICATION_FAILED':
      if (state.phase !== 'DEPLOY_VERIFY') return reject(`deployment verification is not due in phase ${state.phase}`);
      manifest.state.status = 'DEPLOYMENT_FAILED';
      stamp();
      return { applied: true, status: manifest.state.status };

    case 'WAVE_COMPLETED': {
      if (state.phase !== 'DEPLOY_VERIFY') return reject(`wave completion is not due in phase ${state.phase}`);
