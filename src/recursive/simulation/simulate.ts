...
function fixtureReleaseReceipt(input: {
  status: "succeeded" | "failed";
  missingGates?: ReleaseReceipt["missing_gates"];
  commitSha: string;
  wave: number;
}): ReleaseReceipt {
  const evidenceRef = {
 ...
 */
export async function runSimulatedThreeWaveProof(
  ...
  try {
    ...
    const WAVE_DEFECTS: Record<number, { stage: string; errorCode: string; message: string }> = {
      1: {
        stage: "visual-qa",
        errorCode: "SIM_VISUAL_DEFECT",
        message: "simulated quality-model defect",
      },
      2: {
        stage: "site-build",
        errorCode: "SIM_BUILD_DEFECT",
        message: "simulated correctness defect",
      },
      3: {
        stage: "design-intelligence",
        errorCode: "SIM_DESIGN_DEFECT",
        message: "simulated capability defect",
      },
    };
    ...
    const deployment = new DeploymentVerifier(
      new LocalDirectoryDeploymentAdapter(deployDir, healthMarker),
    );
    ...
    const patchInstructionFor = (w: 1 | 2 | 3): PatchInstruction => ({
      ...
      apply: (workdir: string) => {
        ...
        writeFileSync(
          join(workdir, `src/sim-marker-${w}.ts`),
          `// simulated bounded change for wave ${w}\nexport const marker = ${w};\n`,
          "utf-8",
        );
      },
    ...
    const replayProvider = (input: {
      pack: import("../contracts/types.js").PEPack;
      ...
      return {
        originating: [
          {
            caseRef: {
              caseId: `REG-SIM-${input.pack.wave}`,
              ref: {
                refKind: "regression-case",
                refId: `REG-SIM-${input.pack.wave}`,
                digest: sha256Text(`REG-SIM-${input.pack.wave}`),
              },
            },
        ...
        controls: [
          {
            caseRef: {
              caseId: "CTRL-SIM-CONTRACT",
              ref: {
                refKind: "control-case",
                refId: "CTRL-SIM-CONTRACT",
                digest: sha256Text("CTRL-SIM-CONTRACT"),
              },
            },
        ...
        disconfirm: [
          {
            caseRef: {
              caseId: "DIS-SIM-UNRELATED",
              ref: {
                refKind: "disconfirm-case",
                refId: "DIS-SIM-UNRELATED",
                digest: sha256Text("DIS-SIM-UNRELATED"),
              },
            },
            beforeResult: existsSync(join(input.beforeWorkdir, "src/recursive/state/constants.ts"))
              ? "PRESENT"
              : "ABSENT",
            afterResult: existsSync(join(input.patchedWorkdir, "src/recursive/state/constants.ts"))
              ? "PRESENT"
              : "ABSENT",
            expectedDirection: "UNCHANGED" as const,
          },
    ...
    const baselineContractDigest = sha256Text(
      readFileSync(join(repository, "package.json"), "utf-8"),
    );
    const holdoutProvider = (
      patchedWorkdir: string,
    ): Array<{ caseId: string; passed: boolean }> => [
      {
        ...
        passed:
          existsSync(join(patchedWorkdir, "package.json")) &&
          sha256Text(readFileSync(join(patchedWorkdir, "package.json"), "utf-8")) ===
    ...
    const receipt = await controller.start({
      ...
      patchInstructions: {
        1: patchInstructionFor(1),
        2: patchInstructionFor(2),
        3: patchInstructionFor(3),
      },
      now: input.now,
    });
