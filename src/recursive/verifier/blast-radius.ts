...
export function computeBlastRadius(input: {
  ...
  const beforeByPath = new Map(
    input.before.map((snapshot) => [snapshot.relativePath, snapshot.sha256]),
  );
  const afterByPath = new Map(
    input.after.map((snapshot) => [snapshot.relativePath, snapshot.sha256]),
  );
  ...
  const unexpectedlyChanged = input.expectedUnchangedArtifacts.filter((path) =>
    changedSet.has(path),
  );
  const expectedUnchanged = input.expectedUnchangedArtifacts.filter(
...
export function snapshotFiles(input: { root: string; paths: string[] }): ArtifactSnapshot[] {
