...
export function buildQualityDimensionResult(
  input: QualityDimensionResultInput,
): QualityDimensionResult {
...
export function qualityDimensionResultRef(result: QualityDimensionResult): LearningArtifactRef {
  ...
  return {
    artifact_type: "QualityDimensionResult",
    artifact_id: `QualityDimensionResult:${digest}`,
...
export function validateQualityDimensionResult(value: unknown): string[] {
  ...
  const errors: string[] = [];
  if (result.schema !== "website-bot.quality-dimension-result/v1")
    errors.push("schema must be website-bot.quality-dimension-result/v1");
  ...
  if (typeof result.candidate_id !== "string" || !result.candidate_id)
    errors.push("candidate_id required");
  if (typeof result.campaign_id !== "string" || !result.campaign_id)
    errors.push("campaign_id required");
