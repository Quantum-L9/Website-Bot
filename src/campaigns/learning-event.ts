...
export function confidenceClassOf(input: {
  ...
  if (input.causal_isolation && input.confirmations >= 2 && input.contradictions === 0)
    return "HIGH";
  if (input.causal_isolation && input.confirmations >= 1 && input.contradictions === 0)
    return "MEDIUM";
  return "LOW";
}
...
export function buildLearningEvent(input: LearningEventInput): LearningEvent {
  ...
  const causal_isolation = Boolean(
    input.counterfactual_pair && input.counterfactual_pair.unchanged.length > 0,
  );
  const payload: Omit<LearningEvent, "integrity"> = {
    ...
    side_effects: Object.fromEntries(
      Object.entries(input.side_effects).map(([key, value]) => [
        assertQualityDimension(key),
        value,
      ]),
...
export function isNegativeLearning(event: LearningEvent): boolean {
  return (
    event.outcome === "REJECTED" || event.outcome === "CONTRADICTED" || event.anti_pattern !== null
  );
}
