...
function deepConvertKeys(value: unknown, convert: (key: string) => string): unknown {
  ...
  return Object.fromEntries(
    Object.entries(value as JsonObject).map(([key, child]) => [
      convert(key),
      deepConvertKeys(child, convert),
    ]),
  );
}

const ARTIFACT_KEYS: Record<
  Exclude<
    EvidenceKind,
    "release" | "handoff" | "registration_ack" | "source_site" | "image_plan" | "image_assets"
  >,
...
> = {
  ...
  build: {
    ...
    buildId: "build_id",
    clientId: "client_id",
    assemblyManifestSha256: "assembly_manifest_sha256",
  ...
  publication: {
    ...
    buildId: "build_id",
    clientId: "client_id",
    buildProofId: "build_proof_id",
    buildProofSha256: "build_proof_sha256",
    repositoryId: "repository_id",
    previousHeadSha: "previous_head_sha",
    commitSha: "commit_sha",
    treeSha: "tree_sha",
    verifiedBranchHeadSha: "verified_branch_head_sha",
    sourceDigest: "source_digest",
    managedManifestDigest: "managed_manifest_digest",
  ...
  deployment: {
    ...
    requestedCommitSha: "requested_commit_sha",
    observedCommitSha: "observed_commit_sha",
  ...
  failure: {
    ...
    sanitizedDetails: "sanitized_details",
    failedAt: "failed_at",
  },
};
...
function remap(value: unknown, map: Record<string, string>, reverse = false): unknown {
  ...
  return Object.fromEntries(
    Object.entries(source).map(([key, child]) => [mapping[key] ?? key, child]),
  );
}
...
export function decodeEvidenceArtifact<T>(kind: EvidenceKind, value: unknown): T {
  ...
  if (kind === "build" && Array.isArray(decoded.checks)) {
    decoded.checks = decoded.checks.map((check) =>
      remap(check, { durationMs: "duration_ms" }, true),
    );
  }
  return decoded as T;
}

export function encodeCheckpoint(value: StageCheckpoint): object {
  return remap(value, CHECKPOINT_KEYS) as object;
}
export function decodeCheckpoint(value: unknown): StageCheckpoint {
  return remap(value, CHECKPOINT_KEYS, true) as StageCheckpoint;
}
export function encodeRecord(value: EvidenceRecord): object {
  return remap(value, RECORD_KEYS) as object;
}
export function decodeRecord(value: unknown): EvidenceRecord {
  return remap(value, RECORD_KEYS, true) as EvidenceRecord;
}
...
export function encodeIndex(index: EvidenceIndex): object {
  return {
    ...
    artifacts: Object.fromEntries(
      Object.entries(index.artifacts).map(([kind, record]) => [
        kind,
        record ? encodeRecord(record) : record,
      ]),
    ),
...
export function decodeIndex(value: unknown): EvidenceIndex {
  ...
  return {
    ...
    artifacts: Object.fromEntries(
      Object.entries((source.artifacts ?? {}) as JsonObject).map(([kind, record]) => [
        kind,
        record ? decodeRecord(record) : record,
      ]),
    ),
    failure_history: Array.isArray(source.failure_history)
      ? source.failure_history.map(decodeRecord)
      : [],
  };
}
