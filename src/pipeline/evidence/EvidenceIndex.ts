...
const VALID_STATUSES = new Set<EvidenceChainStatus>([
  "empty",
  "assembling",
  "built",
  "published",
...
function validateRecord(record: EvidenceRecord, expectedKind?: string): void {
  if (expectedKind && record.kind !== expectedKind)
    throw new Error(`evidence index record kind mismatch for ${expectedKind}`);
  if (
    !record.kind ||
    !record.schema ||
    !record.logicalId ||
    ...
    throw new Error("evidence index record identity is invalid");
  if (!SHA256.test(record.sha256))
    throw new Error(`evidence index digest is invalid for ${record.kind}`);
...
function validateEvidenceIdentity(index: Partial<EvidenceIndex>): void {
  ...
  if (!Number.isInteger(index.revision) || Number(index.revision) < 1)
