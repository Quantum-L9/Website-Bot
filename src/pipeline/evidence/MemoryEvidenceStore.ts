...
export class MemoryEvidenceStore implements EvidenceStore {
  ...
  async transitionStageSucceeded(_stage: string) {
    return this.index;
  }
  async transitionStageFailed(_stage: string, _failure: EvidenceRecord) {
    return this.index;
  }
  async transitionRunConverged() {
    return this.index;
  }
  async validateChain(): Promise<EvidenceChainValidation> {
    return {
      valid: false,
      mode: this.index.mode,
      checkedAt: new Date().toISOString(),
      gates: [],
      identities: {},
      errors: ["plan mode has no persisted evidence"],
    };
  }
  async referenceFor(_kind: EvidenceKind) {
    return undefined;
  }
  async verifyReference(_reference: EvidenceReference) {
    return false;
  }
  async writeAssembly(_value: AssemblyManifest): Promise<EvidenceRecord> {
    return this.forbidden();
  }
  async readAssembly(): Promise<StoredEvidence<AssemblyManifest> | undefined> {
    return undefined;
  }
  async writeBuild(_value: BuildProof): Promise<EvidenceRecord> {
    return this.forbidden();
  }
  async readBuild(): Promise<StoredEvidence<BuildProof> | undefined> {
    return undefined;
  }
  async writePublication(_value: PublicationEvidence): Promise<EvidenceRecord> {
    return this.forbidden();
  }
  async readPublication(): Promise<StoredEvidence<PublicationEvidence> | undefined> {
    return undefined;
  }
  async writeDeployment(_value: DeploymentEvidence): Promise<EvidenceRecord> {
    return this.forbidden();
  }
  async readDeployment(): Promise<StoredEvidence<DeploymentEvidence> | undefined> {
    return undefined;
  }
  async writeReleaseReceipt(_value: ReleaseReceipt): Promise<EvidenceRecord> {
    return this.forbidden();
  }
  async readReleaseReceipt(): Promise<StoredEvidence<ReleaseReceipt> | undefined> {
    return undefined;
  }
  async writeHandoff(_value: WebsiteFactoryHandoffV3): Promise<EvidenceRecord> {
    return this.forbidden();
  }
  async readHandoff(): Promise<WebsiteFactoryHandoffV3 | undefined> {
    return undefined;
  }
  async writeRegistrationAck(_value: SeoBotRegistrationAck): Promise<EvidenceRecord> {
    return this.forbidden();
  }
  async readRegistrationAck(): Promise<SeoBotRegistrationAck | undefined> {
    return undefined;
  }
  async writeSourceSite(_value: SourceSiteManifest): Promise<EvidenceRecord> {
    return this.forbidden();
  }
  async readSourceSite(): Promise<StoredEvidence<SourceSiteManifest> | undefined> {
    return undefined;
  }
  async writeImagePlan(_value: ImageAssetPlan): Promise<EvidenceRecord> {
    return this.forbidden();
  }
  async readImagePlan(): Promise<StoredEvidence<ImageAssetPlan> | undefined> {
    return undefined;
  }
  async writeImageAssets(_value: ImageAssetManifest): Promise<EvidenceRecord> {
    return this.forbidden();
  }
  async readImageAssets(): Promise<StoredEvidence<ImageAssetManifest> | undefined> {
    return undefined;
  }
  async writeFailure(_value: StageFailureEvidence): Promise<EvidenceRecord> {
    return this.forbidden();
  }
  ...
  async writeCheckpoint(_value: StageCheckpoint): Promise<string> {
    return this.forbidden();
  }
  async readCheckpoint(_stage: string): Promise<StageCheckpoint | undefined> {
    return undefined;
  }
  async loadValidatedReleaseBundle(
    _options?: LoadReleaseBundleOptions,
