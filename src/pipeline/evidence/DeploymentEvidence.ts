...
export function validateDeploymentEvidence(value: unknown): asserts value is DeploymentEvidence {
  ...
  if (
    evidence.schema !== "website-bot.deployment-evidence/v2" ||
  ...
  if (!SHA256.test(String(evidence.publicationSha256)))
    throw new Error("deployment publication digest is invalid");
  if (
    !SHA1.test(String(evidence.requestedCommitSha)) ||
    !SHA1.test(String(evidence.observedCommitSha))
  ) {
    throw new Error("deployment commit identity is invalid");
  }
  if (evidence.requestedCommitSha !== evidence.observedCommitSha)
    throw new Error("deployment commit correlation failed");
  if (!REPOSITORY.test(String(evidence.sourceRepository)) || !evidence.sourceBranch)
    throw new Error("deployment source identity is invalid");
  if (
    !["api", "deploy_hook"].includes(String(evidence.triggerMode)) ||
    !["preview", "production"].includes(String(evidence.target))
  ) {
  ...
  if (
    !Array.isArray(evidence.aliases) ||
    evidence.aliases.some((alias) => typeof alias !== "string")
  )
    throw new Error("deployment aliases are invalid");
  if (!evidence.readyAt || Number.isNaN(Date.parse(evidence.readyAt)))
    throw new Error("deployment readyAt is invalid");
}
