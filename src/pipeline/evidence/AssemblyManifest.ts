...
export function validateAssemblyManifest(value: unknown): asserts value is AssemblyManifest {
  ...
  if (manifest.schema !== 'website-bot.assembly-manifest/v2' || !manifest.buildId || !manifest.clientId || !manifest.generatorVersion || !manifest.templateVersion) {
  ...
  if (!SHA256.test(String(manifest.templateDigest))) throw new Error('assembly template digest is invalid');
