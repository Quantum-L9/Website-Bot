...
const REQUIRED_CHECKS: BuildCheckName[] = [
  "install",
  "astro-check",
  "astro-build",
  "route-assertion",
...
export function validateBuildProof(value: unknown): asserts value is BuildProof {
  ...
  if (
    proof.schema !== "website-bot.build-proof/v2" ||
    proof.status !== "passed" ||
    !proof.proofId ||
    !proof.buildId ||
    !proof.clientId
  ...
  if (
    !SHA256.test(String(proof.assemblyManifestSha256)) ||
    !SHA256.test(String(proof.sourceDigest)) ||
    !SHA256.test(String(proof.distDigest))
  ) {
    throw new Error("build proof digest is invalid");
  }
  if (proof.packageManager !== "npm" || !proof.packageManagerVersion)
    throw new Error("build proof runtime is incomplete");
  ...
  for (const required of REQUIRED_CHECKS)
    if (!names.has(required)) throw new Error(`build proof missing ${required}`);
  if (
    proof.checks.some(
      (check) =>
        check.status !== "passed" || !Number.isFinite(check.durationMs) || check.durationMs < 0,
    )
  ) {
    throw new Error("build proof check result is invalid");
  }
  if (!Array.isArray(proof.builtRoutes) || proof.builtRoutes.length === 0)
    throw new Error("build proof routes are missing");
  if (
    !proof.startedAt ||
    Number.isNaN(Date.parse(proof.startedAt)) ||
    !proof.completedAt ||
