...
const GATES = new Set<ReleaseGate>([
  "assembly",
  "local_build",
  "github_publication",
  "vercel_deployment",
...
export function validateReleaseReceipt(value: unknown): asserts value is ReleaseReceipt {
  ...
  if (
    !MODES.has(receipt.mode as ExecutionMode) ||
    !STATUSES.has(receipt.status as ReleaseReceiptStatus)
  ) {
  ...
  if (receipt.correlation.dist_digest && !SHA256.test(receipt.correlation.dist_digest))
  ...
  if (
    ...
    !["pending", "passed", "failed", "skipped"].includes(String(receipt.qa.visual_qa))
  ) {
