...
export function validateStageFailureEvidence(
  value: unknown,
): asserts value is StageFailureEvidence {
  ...
  if (
    failure.schema !== "website-bot.stage-failure/v2" ||
    !failure.buildId ||
    !failure.clientId ||
    !failure.stage ||
    !failure.code ||
  ...
  if (!Number.isInteger(failure.attempt) || Number(failure.attempt) < 1)
    throw new Error("failure evidence attempt is invalid");
  ...
  if (!failure.failedAt || Number.isNaN(Date.parse(failure.failedAt)))
    throw new Error("failure evidence timestamp is invalid");
}
