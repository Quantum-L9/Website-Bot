...
import type { EvidenceChainStatus, EvidenceIndex } from "./EvidenceIndex.js";
...
export function transitionStageFailed(
  index: EvidenceIndex,
  stage: string,
...
export function transitionStageSucceeded(
  index: EvidenceIndex,
  stage: string,
  now: string,
): EvidenceIndex {
...
export function transitionRunConverged(index: EvidenceIndex, now: string): EvidenceIndex {
  if (index.failed_stage || index.artifacts.failure)
    throw new Error("cannot converge evidence chain with an active failure");
