...
function validateSeoContract(
  ...
  if (contract.lead_form_action !== undefined) {
    if (
      typeof contract.lead_form_action !== "string" ||
      contract.lead_form_action.trim().length === 0
    ) {
      errors.push("seo_contract.lead_form_action, when present, must be a non-empty string");
    } else {
      let parsed: URL | undefined;
      try {
        parsed = new URL(contract.lead_form_action);
      } catch {
        parsed = undefined;
      }
      check(
    ...
    if (!isObject(sourceSite)) {
      ...
      if (typeof sourceSite.url !== "string" || sourceSite.url.trim().length === 0) {
        ...
        if (!parsed || (parsed.protocol !== "https:" && parsed.protocol !== "http:")) {
          ...
          errors.push(
            "assets.sourceSite.url resolves to a forbidden (local/private/metadata) host",
          );
        }
      }
      check(
      ...
      providedImages.forEach((image, index) => {
        ...
        check(
          image.intendedPlacement === undefined ||
            (typeof image.intendedPlacement === "string" &&
              image.intendedPlacement.trim().length > 0),
      ...
      imageSlots.forEach((slot, index) => {
        ...
        if (slot.generation !== undefined) {
          if (!isObject(slot.generation))
            ...
            check(
              typeof slot.generation.intent === "string" &&
                slot.generation.intent.trim().length > 0,
              `assets.imageSlots[${index}].generation.intent must be a non-empty string`,
            );
        }
        if (typeof slot.id === "string") {
          if (seenIds.has(slot.id))
            errors.push(`assets.imageSlots contains duplicate id ${slot.id}`);
          seenIds.add(slot.id);
        }
    ...
    if (!isObject(deploy)) {
      ...
      check(
        deploy.publish_credential_ref === undefined ||
          (typeof deploy.publish_credential_ref === "string" &&
            /^env:\/\/[A-Z][A-Z0-9_]*$/.test(deploy.publish_credential_ref)),
      ...
      check(
        deploy.seo_bot_github_credential_ref === undefined ||
          (typeof deploy.seo_bot_github_credential_ref === "string" &&
            /^env:\/\/[A-Z][A-Z0-9_]*$/.test(deploy.seo_bot_github_credential_ref)),
        "deploy.seo_bot_github_credential_ref, when present, must be env://NAME",
      );
      check(
        deploy.seo_bot_vercel_deploy_hook_ref === undefined ||
          (typeof deploy.seo_bot_vercel_deploy_hook_ref === "string" &&
            /^env:\/\/[A-Z][A-Z0-9_]*$/.test(deploy.seo_bot_vercel_deploy_hook_ref)),
    ...
    if (!isObject(provision)) {
      ...
      if (!isObject(github)) {
        ...
        check(
          github.repository === undefined ||
            (typeof github.repository === "string" &&
              /^[A-Za-z0-9_.-]{1,100}$/.test(github.repository)),
        ...
        check(
          github.publish_credential_ref === undefined ||
            (typeof github.publish_credential_ref === "string" &&
              /^env:\/\/[A-Z][A-Z0-9_]*$/.test(github.publish_credential_ref)),
        ...
        if (vercel.environment !== undefined) {
          if (!Array.isArray(vercel.environment)) {
            ...
            vercel.environment.forEach((entry, index) => {
              ...
              check(
                typeof entry.value_ref === "string" &&
                  /^env:\/\/[A-Z][A-Z0-9_]*$/.test(entry.value_ref),
                `provision.vercel.environment[${index}].value_ref must be env://NAME`,
              );
              check(
                entry.type === undefined ||
                  ["plain", "encrypted", "sensitive"].includes(String(entry.type)),
      ...
      if (maintenance !== undefined) {
        if (!isObject(maintenance)) errors.push("provision.maintenance must be an object");
          ...
          check(
            maintenance.github_credential_ref === undefined ||
              (typeof maintenance.github_credential_ref === "string" &&
                /^env:\/\/[A-Z][A-Z0-9_]*$/.test(maintenance.github_credential_ref)),
            "provision.maintenance.github_credential_ref must be env://NAME",
          );
          check(
            maintenance.vercel_deploy_hook_ref === undefined ||
              (typeof maintenance.vercel_deploy_hook_ref === "string" &&
                /^env:\/\/[A-Z][A-Z0-9_]*$/.test(maintenance.vercel_deploy_hook_ref)),
            "provision.maintenance.vercel_deploy_hook_ref must be env://NAME",
          );
        }
      }
      check(
