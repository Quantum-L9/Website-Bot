...
export const websitePipeline = inngest.createFunction(
  ...
  async ({ event, step }: { event: { data: PipelineRequestedData }; step: any }) => {
    ...
    return await step.run("run-evidence-backed-pipeline", async () => {
      ...
      const shouldProvision = Boolean(
        data.provision || (!spec.deploy && spec.provision?.enabled !== false && spec.provision),
      );
      ...
      try {
        ...
        return {
          ...
          mode,
          evidenceRoot,
          chainStatus: index.chain_status,
          releaseReceipt: release ?? null,
        };
      } catch (error) {
        if (error instanceof BudgetExceededError || error instanceof AdmissionRejectedError)
