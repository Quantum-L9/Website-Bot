...
export function evaluateSourceImage(
  ...
  if (inspected.byteLength > policy.maxBytes) {
    return { accepted: false, reason: `exceeds max bytes (${inspected.byteLength} > ${policy.maxBytes})` };
  }
  ...
  if (inspected.mimeType !== 'image/svg+xml') {
    ...
    if (inspected.width > 0 && inspected.height > 0) {
      const ratio = Math.max(inspected.width / inspected.height, inspected.height / inspected.width);
      if (ratio > policy.maxAspectRatio) return { accepted: false, reason: `extreme aspect ratio (${ratio.toFixed(1)}:1)` };
    }
  }
  return { accepted: true };
}

const TRACKING_PARAM = /^(utm_|fbclid$|gclid$|mc_|ref$|ref_src$|igshid$|_ga$|sessionid$|sid$|phpsessid$|jsessionid$)/i;
...
const SKIP_PATH = /(\/(login|logout|signin|signout|register|account|cart|checkout|basket|search|wishlist)(\/|$|\?))|(\/(tag|tags|page|category)\/\d+)/i;
...
export function shouldSkipUrl(rawUrl: string, options: { allowPdf?: boolean } = {}): boolean {
  ...
  if (!options.allowPdf && /\.(pdf|zip|dmg|exe|mp4|mov|avi|woff2?|ttf|eot)$/i.test(path)) return true;
