...
export class SourceCrawler {
  ...
  private async assertResolvedSafe(hostname: string): Promise<void> {
    ...
    for (const record of records) {
      if (isForbiddenAddress(record.address)) throw new Error(`hostname ${hostname} resolves to forbidden address ${record.address}`);
    }
  }

  private pageValidator = async (url: string): Promise<void> => {
    if (this.opts.allowPrivateHosts) { assertProtocol(url); return; }
    const parsed = assertUrlAllowed(url, { seedHost: this.seedHost, allowSubdomains: this.opts.allowSubdomains });
  ...
  private inScope(url: string): boolean {
    ...
    return isUrlAllowed(url, { seedHost: this.seedHost, allowSubdomains: this.opts.allowSubdomains });
  }
  ...
  async crawl(): Promise<SourceSiteManifest> {
    ...
    const pageFetcher = this.opts.fetcher ?? new HttpPageFetcher({ validateUrl: this.pageValidator, navigationTimeoutMs: this.opts.navigationTimeoutMs ?? DEFAULTS.navigationTimeoutMs });
    const imageFetcher = this.opts.fetcher ?? new HttpPageFetcher({ validateUrl: this.imageValidator, navigationTimeoutMs: this.opts.navigationTimeoutMs ?? DEFAULTS.navigationTimeoutMs, maxBytes: this.policy.maxBytes });
    ...
    while (queue.length > 0 && pages.length < maxPages) {
      ...
      if (depth < maxDepth) {
        for (const link of extracted.links) {
          ...
          if (visited.has(normalized) || !this.inScope(normalized) || shouldSkipUrl(normalized, { allowPdf: this.opts.allowPdf })) continue;
      ...
      for (const candidate of extracted.images) {
        ...
        try {
          ...
          rejected.push({ sourceUrl: candidate.url, referringPageUrl: fetched.finalUrl, reason: `download failed: ${String(error)}` });
        }
      }
    }
...
function assertProtocol(url: string): void {
  ...
  if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') throw new Error(`protocol not allowed: ${parsed.protocol}`);
}
