...
function buildTargetKeywords(
  spec: DomainSpec,
): Array<{ keyword: string; priority: KeywordPriority }> {
  ...
  if (Array.isArray(raw)) {
    const values = raw.flatMap((item): Array<{ keyword: string; priority: KeywordPriority }> => {
      if (typeof item === "string" && item.trim())
        return [{ keyword: item.trim(), priority: "medium" }];
      ...
      return [
        {
          ...
          priority: keywordPriority((item as { priority?: unknown }).priority),
        },
      ];
    });
...
export function buildWebsiteFactoryHandoffV3(
  input: WebsiteFactoryHandoffBuildInput,
  ...
  if (!releaseBundle.validation.valid)
    throw new Error("v3 handoff requires a valid release evidence bundle");
  if (releaseBundle.index.mode !== "end-to-end")
    throw new Error("v3 handoff requires end-to-end evidence mode");
  if (!buildProof || !publication || !deployment)
    throw new Error("v3 handoff requires build, publication, and deployment evidence");
  if (receipt.status !== "succeeded" || receipt.missing_gates.length > 0)
    throw new Error("release receipt is not complete");
  if (qualitySummary.visualQa !== "passed" || receipt.qa.visual_qa !== "passed")
    throw new Error("v3 handoff requires passed visual QA");
  if (clientId !== releaseBundle.index.client_id || buildId !== releaseBundle.index.build_id)
    throw new Error("handoff identity differs from the release bundle");
  if (domainSpec.client_id !== clientId)
    throw new Error("DomainSpec client identity differs from the release bundle");

  const githubCredentialRef =
    deployTarget.seoBotGithubCredentialRef ??
    process.env.SEO_BOT_SITE_GITHUB_CREDENTIAL_REF ??
    "env://SEO_BOT_SITE_GITHUB_TOKEN";
  const deployHookRef =
    deployTarget.seoBotVercelDeployHookRef ?? process.env.SEO_BOT_SITE_VERCEL_HOOK_REF;
